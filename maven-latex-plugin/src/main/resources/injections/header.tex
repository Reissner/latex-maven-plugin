...
\iftutex%
  ...
  \ifluatex%
    \ifpdf%
    \else
      % solves a problem of datetime2 for lualatex --output-format=dvi. 
      % TBD: check from time to time whether the bug is fixed already 
      \usepackage{luatex85}
    \fi
  \fi
\else
...
\usepackage{csquotes}% use \PassOptionsToPackage{autostyle}{csquotes}
...
\IfClassLoadedTF{beamer}{
  ...
  \IfClassLoadedTF{leaflet}{%
    % maybe here geometry goes into 
  }{%
...
\IfClassLoadedTF{beamer}{
  ...
  \usepackage[aux]{rerunfilecheck}% not used glossary and index 
}

\makeatletter\disable@package@load{auxhook}{}\makeatother% just bugfix: superfluous in next hyperref version 
% hyperref and xcolor 
...
\fi%ifpdf
\makeatletter\reenable@package@load{auxhook}\makeatother% just bugfix: superfluous in next hyperref version 
...
% used to import tex files including graphics: mixed pics 
\usepackage{import}
...
% index: always load package splitindex, except in some classes or if tex4ht. 
% If no index is needed just don't command \makeindex 
\IfClassLoadedTF{scrlttr2}{%
}{
  % index and glossary
  \IfPackageLoadedTF{tex4ht}{
  }{
    \usepackage{splitidx}%[split] is not supported 
    %  \usepackage{makeidx}% superfluous if splitindex loaded 
    %  \usepackage{showidx}
    % \makeindex %invoked by need only 

    % TBD: eliminate; loads amsmath!!! 
    % Concerning this section there is an entry in changes.xml 
    % There are also articles and so which do not need glossaries. 
    % So loading glossaries and invoking \makeglossaries 
    % may not be appropriate. 
    % \makeglossaries emits a warning if no \printglossaries is present 
    % so currently a compromise: load package glossaries 
    % but dont invoke \makeglossaries. 
...
\IfClassLoadedTF{beamer}{
  ...
  \IfClassLoadedTF{leaflet}{%
    % maybe this exclusion is because inclusion is superfluous 
  }{%
    \IfClassLoadedTF{scrlttr2}{%
    }{
...
% wrappers in conjunction with .chktex: ignored 
%\newcommand{\textTT}[1]{\texttt{#1}}
\newcommand{\textttNoChk}[1]{\texttt{#1}}
% chktex executes \input{#1} but does not find the file #1: suppress that warning 
