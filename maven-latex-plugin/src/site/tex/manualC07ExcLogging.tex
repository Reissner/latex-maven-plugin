...
& \texttt{\footnotesize \$conv: '\$actVersion'not in\$expVersionInt} \\
\multicolumn2{|l|}{
\begin{minipage}{0.95\linewidth}
  Indicates that the version of converter \texttt{\$conv} can be detected
  and is \texttt{\$actVersion} but does not fit the expectation which is the version interval \texttt{\$expVersionInt}. 
\end{minipage}
}
\end{longtable}
...
\begin{longtable}{|ll|}
  ...
  \endlastfoot%
  \texttt{\scriptsize TLP01} 
  & \texttt{\scriptsize Artifact '\$pdfFileAct' from '\$texFile' could not be savely reproduced. }  \\
  \multicolumn2{|l|}{
  \begin{minipage}{0.95\linewidth}
  Processing \texttt{\$texFile} yields \texttt{\$pdfFileAct} which is not ``alike'' the stored version 
  or the difference check failed at all. 
  Currently, that kind of check can be performed for PDF files only. 
  Also, the diff check is executed only if parameter \texttt{\$chkDiff} 
  described in Section~\ref{sec:paramRepro} is set. 
  Then the diff command \texttt{\$diffPdfCommand} is performed 
  to determine whether the artifacts are equivalent
  ...
  & \texttt{\footnotesize Add file '\$pdfFileCmp' to compare with artifact '\$pdfFileAct'! }  \\
  \multicolumn2{|l|}{
  \begin{minipage}{0.95\linewidth}
  The PDF file \texttt{\$pdfFileCmp} expected for comparison with the PDF file \texttt{\$pdfFileAct} 
  created from a \LaTeX{} main file does not exist. 
  It is expected only if a diff check is configured 
  according to \texttt{\$chkDiff} described in Section~\ref{sec:paramRepro}. 

  This warning is normal if the document is added newly. 
  Then just copy the created PDF file (maybe preserving modification time) after quality check. 
  This warning is also normal if a document is actively modified. 
  Then before building the file \texttt{\$pdfFileCmp} shall be removed before compilation 
  to force this software to assign a new timestamp, 
  e.g.\@ into first page and metadata. 
...
\multicolumn2{|l|}{
\begin{minipage}{0.95\linewidth}
This indicates that for conversion to PDF there are a DVI-file and a \gls{xdv}-file 
...
The former explanation was as follows
...
described in Section~\ref{subsubsec:openingMagComm} occurs, 
...
\texttt{\footnotesize WLP09} & \texttt{\footnotesize For file '\$texFile' targets are neither specified by magic comment } \\
                             & \texttt{\footnotesize nor restricted by document class '\$docClass'. } \\
 \multicolumn2{|l|}{
 \begin{minipage}{0.95\linewidth}
 Indicates that the \LaTeX{} main file \texttt{\$texFile} 
 has neither a magic comment specifying the targets 
 nor for the document class parameter \texttt{docClassesToTargets} 
 described in Table~\ref{tab:paramGen} specifies the allowed targets. 
 ...
 or specify targets by magic comment. 
 \end{minipage}
 } \\
 \texttt{\footnotesize WLP10} & \texttt{\footnotesize Degraded identifier for '\$file'; } \\
                              & \texttt{\footnotesize augmented risk not to rerun although necessary. } \\
\multicolumn2{|l|}{
\begin{minipage}{0.95\linewidth}
Indicates that an auxiliary file \texttt{\$file} 
which is used to determine whether an auxiliary program shall be rerun could not be completely evaluated. 
An example of an auxiliary file is an IDX file. 
If it changes not only \texttt{makeindex} but also the \LaTeX{} compiler need to be rerun. 

A special kind of auxiliary files are AUX files. 
They may be used to create bibliographies or glossaries. 
They are special in that they may include other AUX files, 
namely those corresponding with included TEX files. 
In this case, \texttt{\$file} is the top level AUX file. 
\end{minipage}
} \\\end{longtable}
