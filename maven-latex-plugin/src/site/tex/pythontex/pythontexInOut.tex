...
Interaction of \pytex{} with a \LaTeX{} engine like \lualatex{} 
is comparable to that of other auxiliary programs like \makeIndex: 
A latex package makes the \LaTeX{} engine extract information for the auxiliary program 
into a separate file or more. 
Then the auxiliary program is run which creates further files 
which the \LaTeX{} engine reads in a second run. 
...
containing the \LaTeX{} main file \texttt{xxx.tex} % TBD: this term shall be in terms.bib. 
...
\begin{itemize}
  ...
  coming from the command \cmd{pys}; 
  ...
  also determined by the command \cmd{pys}, 
...
\bibliography{../lit,../litLMPpub}% chktex 11 
