...
\item[verifyPdf]
A PDF checker verifying conformance to standards like PDF/A. 
\item[diffPdf]
...
and for some converters with more complex options, 
the options are split over more than one setting, 
e.g.\@ for converter category \tool{fig2dev} converting \gls{fig}-files, 
there are general settings given by \texttt{fig2devGenOptions}
and settings specific for the output language: 
\LaTeX{} (\texttt{fig2devPtxOptions}) 
and \gls{eps} (\texttt{fig2devPdfEpsOptions}. 
In any case, options are trimmed, 
i.e.\@ leading and trailing white spaces are removed 
before being processed. 
There are cases, 
where the options as given are not directly passed to the converter 
...
the target \texttt{dvi} yields an output in extended DVI format, i.e.\@ in \gls{xdv}\@. 
...
the declaration of the document class by ``\cmd{documentclass}'' 
or by the old-fashioned ``\cmd{documentstyle}'' 
preceded by a few constructs as comments and in particular magic comments 
and also ``\cmd{DocumentMetadata}'' to control metadata. 

If the file is really a \LaTeX{} main file, 
the pattern extracts the document class and 
if present the declaration of metadata through ``\cmd{DocumentMetadata}'' 
and magic comments. 
...
and may thus be used one to one in the injection \texttt{.latexmkrc} to configure \tool{latexmk}, 
...
and for an alternative approach, see Section~\ref{subsec:patternLatexMainFile}. 
...
\begin{center}
  `\small{\texttt{beamer,leaflet,scrlttr2:chk,pdf,txt}}' 
\end{center}
%
allows for beamer mainly \texttt{pdf} and derived from that \texttt{txt}.  
Checking with \texttt{chk} does not depend on the document class. 
Note that maybe leaflets or letters may work for DVI or \gls{xdv} also, even for word formats and related, 
...
matches the files to be compiled, the so called \LaTeX{} main files. 
Among the correct \LaTeX{} files it matches exactly the \LaTeX{} main files. 
Moreover, it extracts the following pieces of information, 
...
\begin{description}
  ...
  \item[programMagic]
  the \LaTeX{} engine to be used 
  specifically for the according document. 
  This is intended to be specified only 
  if the required engine for the given document deviates 
  ...
  or \texttt{\% !LMP latexmkMagic} % chktex 26
  which is just short for \texttt{\% !LMP latexmkMagic=true}. % chktex 26
  ...
  or \texttt{\% !LMP chkDiff} % chktex 26
  which is just short for \texttt{\% !LMP chkDiff=true}. % chktex 26

  \item[arg] the argument of invoking a function given by \cmd{UseName}. 
  This is quite specal and may be used to ensure reproducible builds. 
  \item[docMetadata] This is the argument of \cmd{DocumentMetadata}. 
  It is used to specify language, the version of the PDF to be created, 
  conformance to various standards like PDF/A and much more. 
  Like \texttt{arg} it may be involved in ensuring reproducible builds. **** 
\end{description}
...
linerange={166-182}, % chktex 8 TBD: why is this not covered by .chktexrc? 
breaklines,
float,
...
\begin{description}
  \item[1]
  The \texttt{\textbackslash A} indicates the start of the file. 
  \item[2--4]
...
\end{description}
  ...
  and may contain a command and end in a comment. 
...
defining variants (preceeded by \cmd[\{etoolbox\}]{RequirePackage}), 
\item 
the commands \cmd{UseName} and above all \cmd{DocumentMetadata} allowing arbitrarily nested braces 
which are to control metadata as described in Section~\ref{subsec:stdDecl} and may be needed to ensure reproducibility 
as described in Section~\ref{sec:chkReprod}, % **** TBD: references 
\item
...
because incorrect files are detected by the \LaTeX{} engine anyway. 
...
or the \LaTeX{} engine in a magic comment 
indicates already a \LaTeX{} main file. 
Whereas the target set makes the document class superfluous, 
this is not the case for the magic comment specifying the \LaTeX{} engine. 


% From the point of view of the \LaTeX{} engine, 
...
absence of first line \texttt{\%~!TEX root=} declaring a TEX file 
as depending on a \LaTeX{} main file 
...
\endlastfoot%
\texttt{fig2devCommand}   & \tool{fig2dev}     \\
\multicolumn2{|l|}{
\begin{minipage}{0.95\linewidth}
...
\midrule
\texttt{gnuplotCommand}   & \tool{gnuplot}     \\
\multicolumn2{|l|}{
\begin{minipage}{0.95\linewidth}
...
\midrule
\texttt{metapostCommand} & \tool{mpost}\sindex[tool]{metapost|see{mpost}}     \\
\multicolumn2{|l|}{
\begin{minipage}{0.95\linewidth}
The command for conversion of gnuplot-files 
...
\begin{minipage}{0.95\linewidth}
   ...
   to invoking the \texttt{\$ebbCommand}, 
   ...
   In 2025 the author realized that the \texttt{\$ebbCommand} \texttt{ebb} 
   was in fact replaced by \texttt{extractbb} 
   and the original command \texttt{ebb} survived just as a link.
   Note also that \miktex{} offers neither package \pkg{bmpsize} 
   nor \texttt{e(xtract)bb}.  % chktex 36
   This alone requires to switch off invocation of \texttt{e(xtract)bb} by default.  % chktex 36
   So the default value is \texttt{false}. 
  \end{minipage}
} \\
\texttt{ebbCommand} & \texttt{extractbb} \\
\multicolumn2{|l|}{
\begin{minipage}{0.95\linewidth}
...
The default value is '\texttt{extractbb}' 
but maybe the installation requires the original '\texttt{ebb}'. 
\end{minipage}
} \\
...
Include the result in \LaTeX{} as: \cmd[\{latexfile.tex\}]{input}. 
...
\endfirsthead%
\bottomrule
\caption{\label{tab:paramLatex2pdf} The \LaTeX{}-to PDF conversion}
\endlastfoot%
\texttt{latex2pdfCommand}       & \lualatex{}      \\
\multicolumn2{|l|}{
\begin{minipage}{0.95\linewidth}
The \LaTeX{} command to create above all a PDF-file with, 
i.e.\@ the \LaTeX{} engine. 
Further formats are DVI and \gls{xdv} and also other formats based on these. 
...
even not creating PDF or other formats via \gls{xdv}\@. 
...
This always uses the dvi-format. 
...
we refrained from subsuming this under ``kind of dvi''. 
...
In particular, there is a table with the options occurring in any \LaTeX{} engine 
and columns indicating for each option for which engines it is valid. 
...
the default options are among the ones common to all supported engines. 
...
This may apply to the main artifact, i.e.\@ PDF or DVI or \gls{xdv}, 
...
\begin{description}
  ...
  \item[{-no-pdf}] generate \gls{xdv} (extended DVI) output rather than PDF\@. 
...
%
\begin{description}
  \item[{-no-pdf}] which is specific to \xelatex, makes \xelatex{} create \gls{xdv} files 
  which currently cannot be further processed by this software. 
  As soon as this software supports \gls{xdv} files, this option is set by this software, 
  not by the user. 
  \item[{-output-format=FORMAT}], which this software uses to set the output format, 
  either to \texttt{dvi} or to \texttt{pdf}. 
  Strictly speaking, this option is supported by all engines, except for \xelatex. 
  ...
  As soon as this software supports \gls{xdv} creation, 
...
In \miktex, all options of all engines are double dash. 
It must be clarified, whether they understand single dash. 
If not one has to clarify whether in \texlive{} all engines understand double dash. 
...
% works also with upmendex with little adaption 
% does not work neither for xindex nor for xindy
\end{minipage}
} \\
...
% The default value is the empty string. 
% upmendex has similar options but neither -t nor -o shall be used. 
\end{minipage}
} \\
...
\multicolumn2{|l|}{
\begin{minipage}{0.95\linewidth}
The pattern is applied line-wise to the log file of the \texttt{\$makeIndexCommand}  
and matching indicates that \texttt{\$makeIndexCommand} failed with an error. 
Note that \texttt{\$makeIndexCommand} is invoked either directly, 
via \texttt{\$splitIndexCommand} or via \texttt{\$makeGlossariesCommand}. 
In the first two cases, the log file is an ilg-file. 
For the last case, not the actual value for \texttt{\$makeIndexCommand} is used 
but the default value. 
% TBD: have a look at correspondence with Nicola Talbot, 
% i remember she wanted to change that. 

The default value for the pattern is chosen 
according to the `\tool{makeindex}' documentation.
% \cite{MkIdxMoe}, Chapter 5.1 
% To adapt to upmendex : 'Error: ' 
% together '^(!! Input index error|Error:)\ '
% Also possible: upmendex code: [1-9][0-9]* errors 
% Together: ([0-9]+ entries accepted, [1-9][0-9]* rejected)
% even [0-9]+ errors becuse comes only if not zero errors 

\end{minipage}
} \\
% TBD: must it be `\ ' instead of mere blank?
...
% \cite{MkIdxMoe}, Chapter 5.2, 
% to adapt to upmendex: 'Warning: ' should be even '^Warning: '
% Together '(\#\# Warning|Warning:)\ '
% upmendex code: ^[1-9][0-9]* warning(s)?, written in .+
% Together: '[1-9][0-9]* warning(s)?'
\end{minipage}
} \\
\texttt{patternReRunMakeIndex} & \\
\multicolumn2{|l|}{
\begin{minipage}{0.95\linewidth}
\textbf{This parameter is deprecated since version 2.1.} 
Rerun check of auxiliary programs do not read the LOG file. 
Details of the present algorithm 
are described in Section~\ref{sec:runRerunAux}. 
...
Besides the options for \tool{makeindex} and \tool{xindy} 
described in Table~\ref{tab:paramMakeGlossaries}, 
the program \tool{makeglossaries} has additional options from which the following are not allowed: 
%
\begin{description}
  \item[-q] which makes no sense in internal build process. 
  When using \tool{latexmk}, that tool adds this option by internal mechanisms 
  so it shall not be added manually either. 
  \item[-n] prevents from execution of proper commands. 
  \item[-o, -d] which expects and places files in locations this software does not expect. 
\end{description}
...
They are described in Section~\ref{sec:settingsMakeGlossaries}. 
Among the options are the options for `\texttt{makeindex}' (not for \texttt{\$makeIndexCommand}) 
and for `\texttt{xindy}' (also hard-coded). 
The aux-file decides on which program is executed 
...
\texttt{\small(\^{}makeglossaries diagnostic messages:\$} \\% chktex 37
\multicolumn2{|l|}{
\begin{minipage}{0.95\linewidth}
\textbf{This parameter is deprecated since version 2.1.} 
The pattern is applied line-wise to each log file of the index processor, 
invoked by \texttt{\$makeGlossairesCommand} which is either \tool{makeindex} or \tool{xindy}. 
It is appended to the log file by \texttt{makeglossaries} in case the according index processor had an error. 

Note that from the point of view of a build tool, 
this is a redundant piece of information. 
It is only written to help an end-user to understand the error message of the index processor better. 
Also note that there is no according message for warnings of the index processor 
and that the above message is written by \tool{makeglossaries} 
but not by \tool{makeglossaries-lite}. 
\end{minipage}
} \\
\texttt{patternErrXindy} & \texttt{(\^{}ERROR:\ )} \\% chktex 36 chktex 37
\multicolumn2{|l|}{
\begin{minipage}{0.95\linewidth}
The pattern is applied line-wise to each log file of the index processor \tool{xindy}, 
invoked by \texttt{\$makeGlossariesCommand}. 


%  The pattern is applied line-wise to the ilg-file 
%and matching indicates a warning \texttt{\$makeIndexCommand} emitted. 
%The default value is chosen 
%saccording to the `\texttt{makeindex}' documentation.

The pattern in each log file 
...
\section{Parameters for a PDF verifyer like \texttt{verapdf}}\label{sec:verapdf}

Among the applications used by this software, 
the PDF verifyer plays a special role: 
It is not really a converter and resembles more a code checker as described in Section~\ref{sec:chkTex}. 
Like for the most converters also for the codechecker 
we can specify the command ant its options, both given in Table~\ref{tab:paramVeraPdf}. 
For a general description of the validation feature consult Section~\ref{sec:stdVerify} 
and in particular Section~\ref{subsec:stdVeri}.  
...
\begin{longtable}{|ll|}
  ...
  \endfirsthead%
  \bottomrule
  \caption{\label{tab:paramVeraPdf} The parameters of a PDF validator }
  \endlastfoot%
  \texttt{verifyStdCommand}      & \texttt{verapdf}        \\
  \multicolumn2{|l|}{
  \begin{minipage}{0.95\linewidth}
  The command for a tool to verify a PDF standard, 
  which can be claimed by a PDF file created from \LaTeX{} 
  which is one of kinds of PDF/A, PDF/X or PDF/UA, WtPDF at the time of this writing 
  or some else in future. 
  It must provide return values 
  %
  \begin{itemize}
  \item[0] for check executed and passed, 
  \item[1] executed but failed, 
  \item and possibly other codes indicating that the test could not be executed, 
  i.e.\@ a failure; these must be in 2--12 except 5. 
  \end{itemize}
  %
  The default options \texttt{\$verifyStdOptions} are adapted to the default command \texttt{verapdf}. 
  That \texttt{verapdf} yields the according return values is documented in~\cite{DocMeta}, Section~3. 
  \end{minipage}
  } \\
  \texttt{verifyStdOptions}      & \texttt{--flavour 0 -{}-format text}  \\
  \multicolumn2{|l|}{
  \begin{minipage}{0.95\linewidth}
  The options for the command \texttt{\$verifyStdCommand}. 
  ...
  %
  %\begin{verbatim}
  \Verb|\DocumentMetadata{...pdfstandard=}|
  %\end{verbatim}
  %
  one shall neither give a profile via \texttt{-{}-profile} or that like, 
  nor a flavor via \texttt{-{}-defaultflavor} or \texttt{-{}-flavor} 
  except flavor 0 which means, 
  to use flavor claimed by the PDF file itself, 
  which is what is specified in the according TEX file. 
  For obvious reason, \texttt{-{}-off}, \texttt{-{}-help}, 
  \texttt{-{}-list} and \texttt{-{}-version} are not allowed either. 

  Note that \texttt{-{}-format} is set to \texttt{text} 
  ...
  \begin{itemize}
  \item
  a comparison with an original is requested 
  either by a magic comment for the latex main file individually 
  or, if not specified, globally, by \texttt{\$chkDiff} 
  \item
  this original artifact exists, means is stored at proper place, so that the comparison is executed and 
  \item
  the comparison between actually created file and original one confirms equality
  \end{itemize}
  %
  then it is assumed, that the original file had been verified and so comparison validates the actual artifact 
  and need not longer be verified using \texttt{\$verifyStdCommand}. 
  Setting this to \texttt{false} forces explicit verification. 
  \end{minipage}
  } \\
\end{longtable}

The options of \texttt{chktex} 
are described in detail in~\cite{VeraPdf} and links therein. 
...
\section{Parameters for \protect\toolg{latexmk} and related}\label{sec:paramLatexmk}
% TBD: \toolg : \sindex[tool]{} needs to be added 
