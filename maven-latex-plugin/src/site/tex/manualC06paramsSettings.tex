...
it is not really a converter and resembles more a code checker as described in Section~\label{sec:chkTex}. 
Like for the most converters also for the codechecker 
we can specify the command ant its options, both given in Table~\ref{tab:paramVeraPdf}. 
Note that this is invoked only if a PDF file is created 
and if the according \LaTeX{} main file specifies a PDF standard via \cmd{DocumentMetadata} 
