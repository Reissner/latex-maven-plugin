...
First \lualatex{} (or some other \LaTeX{} engine) is run to create the raw index. 
...
Also support of hyper references within indices and glossaries seem to be expanded. 
...
Pattern to identify \LaTeX{} main files: 
Documentation: shall not include the environment 
\cmd{documentclass} or \cmd{documentstyle} in an input. 
Also check whether command \cmd{RequiresPackage} makes sense 
and check whether \cmd{newcommand} or \cmd{renewcommand} is possible % chktex 36
or makes sense. 
...
e.g.~on file\sindex[cmd]{documentclass} 
...
The parameter \param{patternReRunLatex} 
