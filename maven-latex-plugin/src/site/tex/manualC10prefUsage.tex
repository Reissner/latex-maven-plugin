...
\begin{itemize}
  ...
  \item \LaTeX{} Distribution \texlive, to be more precise the newest official variant, 
  not the one offered by SuSE tumbleweed. 
  ...
  All tools listed as default tools in the configuration belong to the preferred usage. 
  In particular, the preferred latex processor is \lualatex. 
  % TBD: this may change: 
  % bibtex and friends may be replaced by biber, 
  % makeindex may be replaced by upmendex 
  % makeglossaries may be replaced by bib2gls 
  % These examples are all that are dynamically chosen by the software. 
  % For example bibtex remains in the config. 
  ...
  \item Although Section~\ref{sec:stdVerify} describes various PDF standards, 
  among those PDF/A, PDF/UA and PDF/X, 
  only PDF/A and PDF/UA or a combination are preferred usage and only if this is done using \cmd{DocumentMetadata}. 
  Using packages \pkg{pdfx} or \pkg{hyperxmp} is possible but not preferred. 
  \item 
