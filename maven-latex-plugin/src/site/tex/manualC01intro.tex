...
From the \LaTeX-files, only the so-called \glspl{gls:lmf} 
must be compiled using a so called \gls{gls:le}, 
other \LaTeX{} files occur only as input for the \cmd{input} command. 
...
and thus need special support by this software. 
If there is some format needed but not yet provided, 
please write an email to the author. 

Many graphic file formats require preprocessing, 
i.e.\@ the according files must be processed before processing \LaTeX{} main files, 
%\gls{lmf}
...
Besides \gls{pdf}, the target formats include the web-formats \gls{html} 
and \gls{xhtml}, 
open offices format \gls{odt}, Microsoft's word formats like \gls{docx} 
and finally plain text. 

Uniformity of ant-task\index{ant-task} and a maven-plugin means in particular, 
