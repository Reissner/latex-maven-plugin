...
\newcommand{\goal}[1]{\texttt{#1}\index{goal!#1}}%|texttt{#1}
\newcommand{\tool}[1]{\texttt{#1}\index{tool!#1}}%|texttt{#1}
...
\author{E. Reissner, ernst.reissner@simuline.eu}
\date{\Today}
...
This is some additional text ignored in the presentation 
because of option \texttt{ignorenonframetext} 
but showing up in the handout. 
...
\begin{frame}
  ...
  This comprises running basic tools like \tool{lualatex} and \tool{bibtex} 
...
\begin{frame}
  ...
  \begin{itemize}
    ...
    also provides a separate goal creating them, \goal{grp} 
    \item
...
\begin{frame}
  ...
  \begin{itemize}
    \item
    sources with \tool{chktex} and logs the results in target and goal \goal{chk}%\index{goal!chk} 
    \item
    versions of used tools via goal \goal{vrs}%\index{goal!vrs}
    \item
...
\begin{frame}
  ...
  of \tool{bibtex} including the package \texttt{rerunfilecheck} for \tool{lualatex} and friends.
  ...
  \begin{itemize}
    ...
    \item configuration file \texttt{.chktexrc} for \tool{chktex}.
    \item configuration file \texttt{.latexmkrc} for \tool{latexmk} 
...
\begin{frame}
  ...
  \begin{itemize}
  \item Support \tool{biber} replacing \tool{bibtex} as preferred tool
  \item Support \tool{xindy} replacing \tool{makeindex} as preferred tool
  \item Support \tool{bib2gls} replacing \tool{makeglossary} as preferred tool
  \item Execute \tool{glosstex} if needed
...
\begin{frame}
  ...
  like \alert{\goal{pdf}}, \alert{\goal{odt}} and \alert{\goal{html}}, 
  there is a goal \alert{\goal{cfg}}, 
  ...
  \setbeamercolor{alerted text}{fg=blue}
  One further goal, \alert{\goal{chk}} to perform a check with \tool{chktex} 
  is allowed among the \texttt{targets}. 

  The other goals cannot serve as targets: 
  %
  \begin{description}
    \item[\goal{vrs}] to check versions of tools, 
    \item[\goal{grp}] to create graphic files speeding up use of \tool{latexmk} 
    and 
    \item[\goal{inj}] to inject useful files like headers or config files 
...
% \section{Examples}
% no index for presentation 
% \mode<all>
% \section{Index}

\mode<article>{
% not allowed for presentation itself: 
% no theindex-environment defined in beamer class 
