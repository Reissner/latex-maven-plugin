...
\endlastfoot%
fig             & xfig, vscode     & \texttt{fig2devCommand}, e.g.~\tool{fig2dev}    \\
gnuplot (gp)    & vscode           & \texttt{gnuplotCommand}, e.g.~\tool{gnuplot}          \\
MetaPost (mp)   & vscode           & \texttt{metapostCommand}, e.g.~\tool{mpost}          \\
svg             & inkscape, vscode & \texttt{svg2devCommand}, e.g.~\tool{inkscape}  \\
jpg, png        & gimp             & \texttt{ebbCommand}, e.g.~\tool{extractbb}                   \\
\end{longtable}
...
\begin{itemize}
\item[\pkg{graphicx}]
is the basic graphics package which provides above all the 
command \cmd{includegraphics} 
...
where PDF is embedded into PDF and EPS is embedded into DVI/\gls{xdv}\@. 
...
Section~\ref{sec:fig2dev} shows how \tool{fig2dev} converts fig-files 
...
Note that \xelatex{}, which always creates an intermediate \gls{xdv} file 
...
The packages actually to be loaded and their respective options both depend on the configuration. 
...
A simple but still useful tool to draw figures is \tool{xfig} 
...
Thus, \tool{fig2dev} offers a mixed solution: 
...
via \cmd[\{xxx\}]{includegraphics} (line 2). 
...
we may use \cmd[\{xxx.ptx\}]{input} instead of \cmd[\{yyy\}\{xxx.ptx\}]{import}. 
...
while each invocation allows specifying also specific options, 
...
and not directly via a script like \tool{fig2dev}. 
...
of figures with \tool{fig2dev} 
and the inclusion of the eps-file and of the pdf-file in the ptx-file. 
Note that the \tool{fig2dev}-command is configurable 
...
{Conversion of a FIG file with fig2dev to TEX file including a PDF/EPS.}%
...
via \cmd[\{xxx\}]{includegraphics} without specifying a suffix. 
...
\UseTaggingSocket{extFigAlt}%
{Conversion of a gnuplot file to TEX file including a PDF/EPS.}%
...
\UseTaggingSocket{extFigAlt}%
{A TEX file created from a gnuplot-file.}
...
described in~\cite{MetaUml} for some reason apparent later. 
...
is given by the parameter \param{metapostCommand} 
which defaults to \tool{mpost}, occasionally just \texttt{mp}. 

Each internal variable which can be set in the MP file 
can also be set when invoking \tool{mpost} 
...
{Note that \param{metapostCommand} may also besides \gls{eps} 
...
As for latex processors, also \tool{mpost} records dependencies recursively 
...
how \tool{mpost} converts an \gls{mp}-file \texttt{xxx.mp} 
...
\UseTaggingSocket{extFigAlt}%
{Conversion of an MP- (metapost) file into an MPS-file.}
...
\includegraphics[alt=typical metapost]{F4_05someMetapost.mps}
}
...
\begin{figure}[htb]
  ...
  {% make redefine local 
\UseTaggingSocket{extFigAlt}%
{An example for a metapost file rendered via luamplib for lualatex.}%
{
  \renewcommand{\aleph}{\alpha}
  \inputmpcode{F4_05someMetapost}
}
...
Some research shows,
that \tool{inkscape} in the version current at time of this writing
...
\end{Verbatim}
%
\tool{inkscape} creates a file \texttt{xxx.pdf}
...
\end{taglstlisting}
%
Unlike \tool{fig2dev} and \tool{gnuplot}, 
...
In older versions of \tool{inkscape},
...
for the current version of \tool{inkscape},
...
The author has filed a bug report to the tool{inkscape} team,
...
should be a picture 
  ...
  \UseTaggingSocket{extFigAlt}%
{Conversion of an SVG file with inkscape into a TEX picture including a PDF/EPS file.}%
...
should be a picture 
  ...
  \UseTaggingSocket{extFigAlt}%
{The result of converting some SVG file into TEX including PDF/EPS.}%
...
Among those, is also \tool{inkscape} 
which can be used like \tool{xfig}  
...
but alternatively also using the command \texttt{extractbb}. 
There is some hint, that \pkg{bmpsize} does not work with \texttt{xelatex}. 
So maybe \texttt{extractbb} is the better alternative. 
In older installations, the command was \texttt{ebb}, 
and \texttt{ebb} may have survived as a link to \texttt{extractbb}. 
Else, the command must be adapted in the configuration. 
...
and to be safe, provides a way to invoke \texttt{extractbb} on the file \texttt{xxx.jpg}. 
...
Sizes seem to differ in DVI/\gls{xdv} output after conversion to PDF, 
...
whether to invoke \tool{extractbb}, which is false by default. 
...
and for \tool{htlatex} in conjunction with \tool{dvipdfmx} 
...
\includegraphics[alt=typical jpg]%[width=0.9\textwidth]
...
FIXME\@: At the moment, \tool{htlatex} does not work with pictures at all. 
...
\includegraphics[width=0.9\textwidth, alt=typical png]
{F4_09somePngOboeBaroqueDennerMIR370}%.png
}
\caption{\label{fig:asIsPng}Some PNG-picture, directly included }
\end{figure}

Note that in DVI/\gls{xdv} mode all usual \LaTeX{} engines 
can include BMP-pictures, whereas in PDF mode only \texttt{xelatex} 
can do that, maybe because it creates \gls{xdv} internally in any case. 
...
%   should be a picture 
%   }{
%   \includegraphics[width=0.9\textwidth, alt=typicalbmp]
