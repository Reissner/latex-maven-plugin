%\DocumentMetadata{lang=en-US,pdfversion=1.7,pdfstandard=a-3b}%,tagging=on,pdfversion=2.0,pdfversion=1.7,dfversion=2.0,pdfstandard=a-4}%pdfversion=1.7,pdfstandard=a-2b}%lang=en-USlang=en,
%,tagging=on,pdfversion=2.0,pdfversion=1.7,dfversion=2.0,pdfstandard=a-4}%pdfversion=1.7,pdfstandard=a-2b}%lang=en-USlang=en,
% a-4 says: switching t a-4f because embedding non-pdf pictures.... Means, a-4 is not a-4f
% Note: a-4f ok, but a-4e (interesting for engineering) is not ok 
%\input{seed}
%\UseName{sys_gset_rand_seed:n}{42}
%tagging-setup={role/user-NS=latex},
\DocumentMetadata{lang=en-US,tagging=on,tagging-setup={role/user-NS=latex},pdfstandard={A-4,UA-2},pdfversion=2.0} %
%tagging=on,,pdfstandard=UA-2,tagging=on,pdfversion=2.0,pdfversion=1.7,dfversion=2.0,pdfstandard=a-4}%pdfversion=1.7,pdfstandard=a-2b}%lang=en-USlang=en,
% Specifying UA-2 works because this just writes required metadata, 
% but validation can pass only with tagging=on, but this document does not work with tagging on. 
% Reaching tagged documents would be another task. 
% NOTE: gnuplot required 1.7; still true? 
...
%   debug={xmp-export},
%   pdfstandard={A-2B},
%   lang={en-US}%, %colorprofiles={A = sRGB.icc}
% }

\PassOptionsToPackage{log}{luaotfload}
...
%DocumentMetadata{uncompressed}
\documentclass[a4paper,12pt,USenglish]{book}%,USenglish unused because already in \DocumentMetadata
%\overfullrule=10mm
...
\usepackage{listings}
% patching of package listing applying to float lstlisting 
% see https://tex.stackexchange.com/questions/755546/understanding-and-avoiding-tagpdf-warning-caused-by-lstlisting-environment
\ExplSyntaxOn
% Internal variable, temporary work-around!
\prop_gput:Nnn\g__tag_float_sect_prop{lstlisting-struct}{\tag_get:n{current_Sect}}
\ExplSyntaxOff

% patching package listings with 
% https://tex.stackexchange.com/questions/755794/problem-with-tagging-non-floating-lstlisting
% https://github.com/latex3/tagging-project/issues/70#issuecomment-2005126960
% providing taglstlisting if non-floating 
\makeatletter
\ExplSyntaxOn
...
\makeatother
% patch for tagging of unnumbered glossaries section 
...
\IfPackageLoadedTF{tex4ht}{
  ...
  % note that this is eliminated from header: there only splitindex is loaded 
  \makeindex
...
%\usepackage{accessibility}% REIE: currently only with pdflatex. but other elements require lualatex. 
...
\newacronym{bb}{BB}{Bounding Box, created by e(xtract)bb } % chktex 36
\newacronym{xbb}{XBB}{Bounding Box, created by e(xtract)bb with the option x.  % chktex 36
...
if used with package \texttt{glossaries} configured for \toolg{makeindex} }
\newacronym{xdy}{XDY}{index style file for \toolg{xindy}: output format of \LaTeX{} engines 
if used with package \texttt{glossaries} configured for \toolg{xindy} }


\newacronym{log}{LOG}{LOGing file: for \LaTeX{} engines and \toolg{mpost}}
\newacronym{aux}{AUX}{auxiliary file: input and output file for \LaTeX{} engines; 
read also e.g\@. by \toolg{bibtex}}
...
\newacronym{synctex}{synctex}{gnu-zipped synchronization files 
relating TEX files and their according PDF files: 
output format of \LaTeX{} engines if used with option \texttt{-synchtex=-1}}

% files in conjunction with bibliographies
\newacronym{bib}{BIB}{BIBliography file: 
In particular input file for the \toolg{bibtex} tool}
\newacronym{bbl}{BBL}{bibliography for a latex document in latex format: 
written by the \toolg{bibtex} tool and read by \LaTeX{} processors}
\newacronym{bst}{BST}{Bibliography Style File read by the \toolg{bibtex} tool}
\newacronym{blg}{BLG}{Bibliography LoG file: for \toolg{bibtex} and related}

% files in conjunction with indices 
\newacronym{idx}{IDX}{InDeX file containing unsorted and multiple index entries; 
output format of \LaTeX{} engines with package \toolg{makeindex} or similar}
\newacronym{ind}{IND}{%
INDex file containing sorted, unified and formatted index entries, 
output format of \toolg{makeindex} and \toolg{xindy}}
\newacronym{ilg}{ILG}{Index LoG file:: for \toolg{makeindex} and related; content depends strongly on the tool}

% files in conjunction with glossaries 
\newacronym{glo}{GLO}{%
GLOssary file containing unsorted and multiple glossary entries; 
output format of \LaTeX{} engines with package \toolg{makeglossaries}}
\newacronym{glg}{GLG}{%
\toolg{makeglossaries} log file }
% TBD: clearify: really only makeglossary? makeglossary-lite? seee ILG
\newacronym{gls}{GLS}{%
glossary file containing sorted, unified and formatted glossary entries;
output format of the \toolg{makeglossaries} tool read by \LaTeX{} engines}
...
output format of \LaTeX{} engines with package \toolg{pythontex}}
...
output format of \LaTeX{} engines with package \toolg{pythontex} 
if loaded with option \toolg{depythontex}
}
\newacronym{plg}{PLG}{%
\toolg{pythontex} log file: home-brewed since the original application does not write log files 
}
\newacronym{dplg}{DPLG}{%
\toolg{depythontex} log file: home-brewed since the original application does not write log files 
}

\newacronym{mp}{MP}{MetaPost: input format for the graphic program \toolg{mpost}}
...
\newindex[Tools related with LaTeX]{tool}
\newindex[LaTeX Packages]{pkg}
\newindex[LaTeX Commands]{cmd}
\newindex[Parameters]{param}
}
...
\NewTaggingSocketPlug{extFigAlt}{default}
 ...
 \AssignTaggingSocketPlug{extFigAlt}{default}

% \AddToHook{cmd/@float@HH/before}{\par\tagstructbegin{tag=float}}
% \AddToHook{cmd/float@endH/after}{\par\tagstructend}
