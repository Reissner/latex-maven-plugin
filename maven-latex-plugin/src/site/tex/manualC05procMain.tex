...
\chapter{Processing of \texorpdfstring{\LaTeX}{LaTeX} Main Files}\label{chap:latexMainConversions}
...
Section~\ref{sec:stdVerify} is special in that it is not related with conversion, 
but it is on specifying and verifying PDF standards, 
with a focus on PDF/A and PDF/UA\@. 
Modern \LaTeX{} compiler support creating PDF artifacts conformant with these standards 
to a wide extent, but this \LaTeX{} builder provides real validation. 
Note that without, the artifact just claims conformance. 

Section~\ref{sec:chkReprod} is also on checking 
but under the aspect of reproducibility. 
...
master TEX files roughly have the form\sindex[cmd]{documentclass} 
%
%\lstset{language=tex, basicstyle=\small}
\begin{taglstlisting}[language=tex, basicstyle=\small]
...
and a file with ending \texttt{synctex.gz} or \texttt{synctex} 
...
Invoking \tool{pythontex} creates by default 
...
\tool{depythontex}, eliminating all python code from the original TEX file. 
...
Replacement is done by \tool{depythontex} 
...
should be a picture 
  ...
  \UseTaggingSocket{extFigAlt}%
{Conversion of a TEX file into PDF, DVI, XDV file using several applications, with auxiliary files created and where they go into.}%
...
should be a picture 
  ...
  \UseTaggingSocket{extFigAlt}%
{Conversion of an AUX file into a BBL file using bibliographies via bibtex.}%
{\import{}{F5_02aux2bbl.ptx}}
}
\caption{\label{fig:aux2bbl}
Conversion of an AUX file into a BBL file using bibliographies}% TBD: how to include \gls in captions? 
\end{figure}
...
%
\begin{taglstlisting}[language=TeX]
\indexentry{ant-task}{3}
\end{taglstlisting}
%
in the \gls{idx} file. 
Caution: If the \gls{idx} file is not open, 
\cmd{index} has no significant impact and in particular no index is created, 
without any warning. 

To create an index, 
a \texttt{makeIndexCommand} described in Table~\ref{tab:paramMakeIndex} on page~\pageref{tab:paramMakeIndex} 
must be run after the \LaTeX{} run. 
...
The manual is~\cite{XindyMan}. 

At time of this writing, quality of output and above all quality of logging of errors and warnings 
given by \tool{makeindex} as described in~\cite{MkIdxMoe}, Section 5 
is not yet fully reached by \tool{upmendex} which is described in~\cite{UpMendex109}. 
Even worse, \tool{xindex}, version 0.61 described in~\cite{Xindex061}
shows poor quality and seemingly does not log errors and messages at all. 
Thus still \tool{makeindex} is preferred usage, as described in Section~\ref{chap:tests}, 
but it is easy to adapt \texttt{patternErrMakeIndex} and \texttt{patternErrMakeIndex} 
also described in Table~\ref{tab:paramMakeIndex} to \tool{upmendex} 
or even so these parameters apply to both \tool{makeindex} and \tool{upmendex}. 
% TBD: this is planned for release 2.4 
% details are in src/changes/Indices 
In the meantime new versions came up, described in ~\cite{UpMendex111} and in ~\cite{Xindex102}, 
respectively. 
Thus, a revaluation may be in place. 



Note that entries in the \gls{idx} file can occur more than once, 
...
to sort the index entries given in the \gls{idx} file 
...
if the \gls{idx} file is empty: \tool{upmendex} does not create an \gls{ind} file at all, 
...
should be a picture 
  ...
  \UseTaggingSocket{extFigAlt}%
{Conversion of an IDX file into an IND file using makeindex.}%
...
\begin{figure}[htb]
  ...
  should be a picture 
    ...
    \UseTaggingSocket{extFigAlt}%
{Conversion of an IDX file into an IND file with makeindex.}%
...
creates lines \cmd[{[y]}\{\dots\}]{indexentry} in the \gls{idx} file 
...
The according entry in the \gls{idx} file is as follows: 
%
\begin{taglstlisting}[language=TeX]
...
Then \tool{splitindex} applies \tool{makeindex} to each of these \gls{idx} files separately 
...
\begin{figure}[htb]
  ...
  should be a picture 
    ...
    \UseTaggingSocket{extFigAlt}%
{Conversion of an IDX file into several IND files using splitindex invoking makeindex.}%
...
This software can decide on the \gls{idx} file whether there is a line specifying an index like so, 
%
\begin{taglstlisting}[language=TeX]
  \indexentry[pkg]{splitidx}{3}
\end{taglstlisting}
...
In addition, it is a conceptual weakness that a glossary database 
...
should be a picture 
  ...
  \UseTaggingSocket{extFigAlt}%
{Conversion of a GLO file into a GLS file using makeglossaries.}%
{\import{}{F5_06glo2gls.ptx}}
}
\caption{\label{fig:glo2gls}Conversion of a GLO file into a GLS file }%using \tool{makeglossaries}
\end{figure}
...
Of course, to that end, 
the interpreter for the desired language must be installed. 
...
Note that like the package \pkg{splitindex}, also \pkg{pythontex} 
comes with an according auxiliary program, 
in this case, besides \tool{pythontex} also \tool{depythontex}. 
...
and if the package is loaded with option \tool{depythontex} 
also a file \texttt{xxx.depytx}.
If the file \texttt{xxx.pytxcode} is present, 
this software invokes the command line tool \tool{pythontex} 
...
Note that this software uses the wrapper \tool{pythontexW} 
of \tool{pythontex} described in Section~\ref{subsec:pythontexW}, 
instead of \tool{pythontex} itself. 
...
As one can see, \tool{pythontex} cooperates with \lualatex{} in a way 
also \tool{bibtex} or the other auxiliary programs do. 
Although \tool{pythontex}, at time of this writing in version 0.18, 
...
This is unlike all the other auxiliary programs in a line with \tool{pythontex}. 
As a consequence, in particular warnings are difficult to detect 
and cannot be detected in a uniform way. 
Thus, the author wrote a little wrapper, called \tool{pythontexW} 
and place it where it can be found, e.g.~in the folder of \tool{pythontex}. 

Accordingly, \tool{depythontex} behaves in a non-standard way: 
...
Also, \tool{depythontex} changes into interactive mode 
...
As \tool{pythontex} also \tool{depythontex} does not write a log file 
but just prints its errors and warnings. 
Thus, the author wrote a little wrapper, 
called \tool{depythontexW} and described in Section~\ref{subsec:pythontexW}, 
and place it where it can be found, e.g.~in the folder of \tool{depythontex}. 
% TBD: evaluate: maybe it is a better solution to enable this software itself to write the output to a log file. 

Unfortunately, as the original author of \pkg{pythontex} shifted his focus 
away from \LaTeX{} towards Markdown, 
he ceased development and so development slowed down next to zero. 
In the meantime, python 2 came out of use 
and having a link called \texttt{python}, 
pointing to either \texttt{python2} or to \texttt{python3} 
as \texttt{(de)pythontex} requires, % chktex 36
went out of fashion. 
So, the user must add the link manually. 
\medskip
...
the auxiliary program \tool{pythontex} executes the python code in the document. 
...
and this is the only information \tool{pythontex} obtains from the package, 
so there is no difference. 

Also, the auxiliary program \tool{pythontex} itself can be configured via command line arguments. 
...
the auxiliary program \tool{pythontex} emits an error, if both are combined. 
...
This software shall invoke \tool{pythontex} 
...
Instead, this software relies on an undocumented feature of auxiliary program \tool{pythontex} 
which is likely not to change: 
If one of the expected output files is missing, 
it recreates all output files, independent of command line options and package options. 
Thus, this software deletes one output file if present, 
before executing \tool{pythontex}. 

When this software invokes \tool{pythontex} 
...
%
\begin{taglstlisting}[basicstyle=\footnotesize]
...
Since \tool{pythontex} is written in python, 
including python code in the \LaTeX{} document 
uses the python interpreter already installed, 
as a prerequisite of \tool{pythontex}. 
To use another language, 
the according interpreter must be installed in addition to python. 
...
\begin{figure}[!htb]
  ...
  should be a picture 
    ...
    \UseTaggingSocket{extFigAlt}%
{Conversion of a pytxcode file using pythontex.}%
{\import{}{F5_07py2dir.ptx}}
  }
  \caption{\label{fig:py2dir}Conversion of a \texttt{pytxcode} file }%using \tool{pythontex}
  \end{figure}


  Figure~\ref{fig:depy2out} shows the files converted by \tool{depythontex}. 
  As for \tool{depythontex}, this software uses the wrapper \tool{depythontexW} 
  of \tool{depythontex} instead of \tool{depythontex} itself. 
  ...
  \begin{figure}[!htb]
    ...
    should be a picture 
      ...
      \UseTaggingSocket{extFigAlt}%
{Conversion of a depytx file using depythontex.}%
{\import{}{F5_08depy2out.ptx}}
    }
    \caption{\label{fig:depy2out}Conversion of a \texttt{depytx} file }%using \tool{depythontex}
    \end{figure}
...
Accordingly, even \tool{pythontex} may need a rerun, 
...
% TBD: add a section on chktex 
\section{Metadata, PDF standards and their verification}\label{sec:stdVerify}

The core of this section is the technique to create PDF artifacts 
conformant with more special standards like PDF/A and to verify conformance. 
A necessary condition for conformance with each of the standards we consider 
is that the artifact contains the claim of the precise standard it conforms with, 
and this claim is part of the metadata. 
Of course, the claim itself is not sufficient but as a rule 
the standards require the presence of various pieces of metadata, 
so e.g.\@ for PDF/A, the title must be given, but may also require absence of metadata 
and there are typically further requirements going beyond metadata. 

We start listing and relating the standards 
which are related to standards being offered by \LaTeX{} engines 
in Section~\ref{subsec:stdPdf}. 
Then we turn our attention to how to tell the \LaTeX{} engines 
to create PDF artifacts 
conform with these standards in Section~\ref{subsec:stdDecl}. 
Requiring these standards is one thing but as the standards are quite complex, 
it is recommended to verify the results. 
Section~\ref{subsec:stdVeri} described how this \LaTeX{} builder supports verification. 
One cannot assume conformance only because compilation passed without error or warning. 
Thus, there are verification tools dedicated to test 
and even these don't gibe full reliability. 



\subsection{Standards based on PDF}\label{subsec:stdPdf}

The most recent PDF file formats are standardized by the ISO, 
version 1.7 in~\cite{Pdf17} and version 2.0 in~\cite{Pdf20}. 
The latter standard mentions further more specific standards based on PDF in its foreword: 
PDF/E is for engineering, 
PDF/X (the X is for eXchange) apply to print industry in general 
and PDF/VT (variable data and transactional printing) is for printing variants of one and the same template 
and for documentation of transactions like bills. 
In contrast, the ISO standards PDF/A for archiving and PDF/UA for unlimited access 
are applied across several industries, public administration, academia and in governmental authorities. 
The main fields of industrial applications are those with legally restricted admission processes 
like medical, military, aviation and other safety and security critical industries, 
where documents must be kept readable for long time. 
Unlimited access refers to access by disabled persons via assistive technologies, 
and is thus required by governmental authorities and other organizations aiming inclusion. 
What is less understood is the fact, that unlimited access also helps search engines 
and technologies based on these to find and index PDF documents. 
The main technology is tagging which describes the logical structure of a document\index{tagging}. 
Tagging is specified in more recent versions of PDF, 
e.g.\@ for PDF 2.0 in~\cite{Pdf20}, Section~14.8. 
PDF/UA imposes requirements beyond mere existence of tagging. 
Since part of these requirements are not easily verifiable automatically\footnote%
{Others may say it is impossible. }, 
the PDF association published a standard on \emph{well-tagged PDF} (WtPDF1),~\cite{WtPDF1}\index{well-tagged} 
which describes, according to its introduction, ``a usage of PDF 2.0 compatible with PDF/UA-2''. 
This seems vague, but means that it is a set of rules 
which suggest a reasonable way to satisfy a considerable set of requirements of the PDF/UA-2 standard. 
Unlike the ISO standards, it is not written from the point of view of users of documents 
but for authors of verification tools. 
Note that WtPDF1 applies to PDF/UA-2, not to PDF/UA-1 which we mention to avoid confusion. 

The PDF association published a series of introductions to ISO standards 
``PDF/X in a nutshell'',~\cite{PDFXnut}, 
``PDF/A in a nutshell'',~\cite{PDFAnut2} and ``PDF/UA in a nutshell'',~\cite{PDFUAnut}, 
providing some history, an overview on the requirements, 
basic techniques used to reach the aims and a view on the role of the standards in society. 

In reality, all specializations of the PDF standard mentioned above, are not single standards 
but families of standards consisting of several \emph{part}s\index{part} published separately. 
As shows Table~\ref{tab:versPdfStd}, roughly speaking, 
the parts of the standards are tied to specific PDF versions. 
For example, PDF/UA-1 is based on PDF 1.7, whereas PDF/UA-2 is based on PDF 2.0. 
But there are also exceptions: Both PDF/A-2 and PDF/A-3 are defined for PDF 1.7. 
The difference is, that PDF/A-2 does not allow embedded files, whereas PDF/A-3 does. 

In many cases, one and the same part defines various\index{conformance level} \emph{conformance level}s. 
For example, PDF/A parts 1,2 and 3 have conformance levels b for basic and a for accessible. 
In addition, Parts 2 and 3 have the level u for Unicode. 
The conformance levels define a hierarchy: level a implies u which implies b. 
Creating a PDF document by scanning allows normally level b only. 
From level u on, cut-and-paste even in mathematical formulae is possible 
and level a provides some accessibility. 
With Part 4 given by~\cite{PdfA4}, which requires PDF 2.0, the system changes: 
PDF/A-4 describes a standard in itself, 
i.e.\@ the absence of a conformance level is in a sense a kind of implicit conformance level 
which allows no embedded files. 
In contrast, the extensions PDF/A-4f (flat) and PDF/A-4e (engineering) 
defined in~\cite{PdfA4}, Annex A and Annex B, respectively, 
allow embedding two- or three-dimensional graphics. 
So, PDF/A-4 is in fact stricter than PDF/A-4f. 
The relation to accessibility was removed completely in PDF/A-4 
in favor of separate standards PDF/UA\@. 
Note that even~\cite{WtPDF1} on well tagged PDF define conformance level r for reproducible 
and a for accessible, whereas neither part of PDF/UA defines a conformance level. 

Table~\ref{tab:versPdfStd} shows the possible combinations between PDF versions 
and versions of standards restricting PDF further. 
PDF/X-2 extends PDF/X-3 and thus is listed after PDF/X-3. 
The standards PDF/VT further restrict the according standards PDF/X. 
The attentive reader will realize that the PDF/X-n standards are published as ISO 15930-m 
but the numbers n and m deviate. 
This is due to the fact, that ISO 15930--2 was never published, ISO 15930--5, partial exchange has been withdrawn 
and that delays prevented publishing in augmenting numbers n. 

Note that PDF/A-4e is the counterpart of PDF/E in a sense: 
PDF/E (engineering) part 1 specifies PDF with 3D graphics for construction for PDF 1.6. 
Part 2 referred to PDF 2.0 but was withdrawn in favor of integrating the functionality into PDF/A-4e. 
PDF/VT makes the PDF/X norms more specific which refer to the according PDF versions. 
So PDF/VT-1,2 specialize PDF/X-4 and PDF/X-5, whereas PDF/VT3 specializes PDF/X-6. 
Note that there are further standards like PDF/R (raster) but these are not considered here, 
because they are not related with the standards which can be specified by \LaTeX{} engines. 

\begin{table}[htb]
  \centering
\begin{tabular}{llccccc}
\toprule
  \multicolumn{2}{l}{PDF standards} & \multicolumn{5}{l}{PDF versions} \\
  Name        & Reference        & 1.3 & 1.4 & 1.6 & 1.7 & 2.0 \\
  \midrule
  \midrule
  PDF/A-1b,a    & \cite{PdfA1}   & --- & x   & --- & --- & --- \\
  PDF/A-2b,a,u  & \cite{PdfA2}   & --- & --- & --- & x   & (x) \\
  PDF/A-3b,a,u  & \cite{PdfA3}   & --- & --- & --- & x   & (x) \\
  PDF/A-4(f,e)  & \cite{PdfA4}   & --- & --- & --- & --- & x   \\
  \midrule
  PDF/UA-1      & \cite{PdfUA1}  & --- & --- & --- & x   & --- \\
  PDF/UA-2      & \cite{PdfUA2}  & --- & --- & --- & --- & x   \\
  \midrule
  PDF/X-1a      & \cite{PdfX1p}  & x   & --- & --- & --- & --- \\% ISO 15930-1 for PDF 1.3
  PDF/X-1a      & \cite{PdfX1}   & --- & x   & --- & --- & --- \\% ISO 15930-4 for PDF 1.4
  PDF/X-3       & \cite{PdfX3p}  & x   & --- & --- & --- & --- \\% ISO 15930-3 for PDF 1.3
  PDF/X-3       & \cite{PdfX3}   & --- & x   & --- & --- & --- \\% ISO 15930-6 for PDF 1.4, complete exchange
  PDF/X-2       & \cite{PdfX2}   & --- & x   & --- & --- & --- \\% ISO 15930-2 never published, ISO 15930-5, partial exchange is withdrawn 
  PDF/X-4,4p    & \cite{PdfX4}   & --- & --- & x   & --- & --- \\% ISO 15930-7, complete exchange
  PDF/X-5g,pg,n & \cite{PdfX5}   & --- & --- & x   & --- & --- \\% ISO 15930-8, partial exchange
  PDF/X-6(p,n)  & \cite{PdfX6}   & --- & --- & --- & --- & x   \\% ISO 15930-9, complete exchange
  \midrule
  PDF/E-1       & \cite{PdfE1}   & --- & --- & x   & --- & --- \\
  PDF/E-2       & \cite{PdfE2}   & --- & --- & --- & --- & x   \\
  PDF/VT1,2     & \cite{PdfVT12} & --- & --- & x   & --- & --- \\
  PDF/VT3       & \cite{PdfVT3}  & --- & --- & --- & --- & x   \\
  \midrule
  WTPDF1r       & \cite{WtPDF1}  & --- & --- & --- & --- & x   \\
  WTPDF1a       & \cite{WtPDF1}  & --- & --- & --- & --- & x   \\
\bottomrule
% need to explain ISO 15930-2 never published, ISO 15930-5, partial exchange is withdrawn 
%  Starting with version 0.95s of pdfmanagement-testphase it is also possible to use
% the values X-4, X-4p, X-5g, X-5n, X-5pg, X-6, X-6n, X-6p, UA-1 for a PDF/X and
% PDF/UA standard. 
  % maybe missing  (see verapdf -l): Well-Tagged PDF 
  % - wt1r WTPDF 1.0 reuse validation profile, 
  % - wt1a WTPDF 1.0 accessibility validation profile 
  % Also missing: PDF/VT (variant type?) VDP variable document printing is a subtype of PDF/X
  % VT-1 X-4
  % VT-2 X-4p X-5G, X-5pg  (above list seems incomplete)
  % VT-2s for streaming 
  % Interesting may be PDF/E (engineering) for which only PDF/E-1 exists and it uses PDF 1.6. 
  % According to https://en.wikipedia.org/wiki/PDF/E for PDF2.0 it is superseded by PDF/A-4e. 
\end{tabular}
\caption{\label{tab:versPdfStd} versions fitting of PDF and standards}
\end{table}

One requirement all these standards have in common is, 
that so called \emph{claimed conformance}, 
is stored in the PDF artifact as metadata in the XMP (Extensible metadata platform) format. 
This is an XML format standardized in~\cite{ISO16684-2}. 
Listing~\ref{lst:metaXmpStd} is part of XMP metadata of a PDF artifact created from \LaTeX{}. 
Observe that the PDF version is 2.0 and that the document is conform with several standards specializing PDF 2.0. 
It is conform with PDF/X-6n, PDF/A-4f, PDF/UA-2 and Well-Tagged PDF WtPDF1 with conformance levels r and a. 

Declared conformance with a part is a prerequisite for conformance, but it is not sufficient. 
Nevertheless, the syntax excludes conformance with two different parts and also with two different conformance levels, 
of a single family be it PDF/X, PDF/A or PDF/UA\@. 
Note also that PDF/A-4, PDF/A-4f and PDF/A-4e exclude one another. 
In contrast, according to~\cite{WtPDF1}, Section~6.1.1, WtPDF1 allows conformance with both conformance levels r and a 
and this is also typical, as both are required for PDF/UA-2. 


\begin{lstlisting}[language=xml,basicstyle=\tiny,
  float, captionpos=b, label={lst:metaXmpStd},
  caption={XMP metadata declaring PDF 2.0, PDF/X-6n, A-4f, UA-2 and WtPDF1a,r}]
<x:xmpmeta xmlns:x="adobe:ns:meta/">
 <rdf:RDF xmlns:rdf="http://www.w3.org/1999/02/22-rdf-syntax-ns#">
  <rdf:Description rdf:about=""
   ...
   <pdf:PDFVersion>2.0</pdf:PDFVersion>
   <pdfxid:GTS_PDFXVersion>PDF/X-6n</pdfxid:GTS_PDFXVersion>
      ...
      <pdfd:claimData>
       <rdf:Bag>
        <rdf:li rdf:parseType="Resource">
         ...
         <pdfd:claimDate>2026-01-06</pdfd:claimDate>
        </rdf:li>
       </rdf:Bag>
      </pdfd:claimData>
     </rdf:li>
...
As Listing~\ref{lst:metaXmpStd} illustrates, one and the same PDF document 
may claim conformance and in fact conform with some PDF/A, PDF/UA and PDF/X in parallel, 
as long as the parts fit the PDF version. 
This is also illustrated in~\cite{PDFaUA}. 

The requirements of the standards going beyond claimed conformance, depend on the individual standard. 
PDF/A for instance is for archiving, so certain metadata are required for indexing in an archive 
as the title and for example the author(s). 
Another component is reproducibility which requires embedded fonts and restricted color schemes. 
For PDF/UA, above all tagging is crucial and being well tagged. 
The author has no PDF/X standard at hand, so the reader must do some research himself/herself 
if interested in details of PDF/X. 

In most cases, the choice of the PDF version restricts the choice of the part and conformance level of the more specific standards. 
In fact, for PDF/UA, there is no choice left. 
For PDF/X only little is publicly known, 
but for PDF/A, the German \foreignlanguage{german}{Bundesarchiv} published the recommendation~\cite{PdfAempf24}. 
The summary is right in Section~1. 
Negative criteria are embedded files in particular of unrestricted formats, image compression and above all javascript. 

The core for comparing parts is Section~2.6, which clearly shows that PDF/A-2 avoids the caveats. 
What is not taken into account is, that PDF/A-4 does not allow embedded files either. 
The arguments in Section~1 exclude just PDF/A-4e and PDF/A-4f. 
What is really a problem is, that PDF/A-4 allows javascript, but this can be excluded in an additional step. 
As the PDF version will increase, one cannot stick to PDF/A-2 which corresponds with PDF 1.7 forever. 



%    for part 3 the conformance level may be missing which is impossible for the other parts. 
%    Conformance is only if part and conformance level coincide. 
%    With special case, 4 for which the lack of explicit conformance level is necessary condition to conform with 4. 

%    Several PDFa part/level declarations are ignored except the last one. 


% For the tagging status of the various packages see 
% \url{https://latex3.github.io/tagging-project/tagging-status/}.

% checker tool rups \url{https://itextpdf.com/products/rups}


% pdftag emits a warning:  WARN PdfMojo - Specified standard ua-1 without tagging; verification will fail. 
% visible on the console. 
% this is strange. 
% but avoids implementing the check in latex builder. 
% further research is required. 



\subsection{Declaring PDF output standards \LaTeX}\label{subsec:stdDecl}


A milestone in XMP support in \LaTeX{} is given in~\cite{XMPlatex} 
which at the same time gives an overview over mutually incompatible techniques for creating artifacts conforming with PDF standards from \LaTeX{}. 
First the state of the art in these days is described which includes a reference to the manual~\cite{PdfxP}, of the \pkg{pdfx} package. 
Although heavily outdated, is worth reading the manual, because the introduction offers a beautiful overview over PDF standards. 
As~\cite{PdfxP}, Section~2.1 shows, the package \pkg{pdfx} supports creation of roughly the standards 
listed in Table~\ref{tab:versPdfStd}. 
What is unique is some support for PDF/VT and for PDF/E. 
On the other hand, only older parts of standards are supported which means that also the PDF versions are restricted. 
Also, the focus is on the old-fashioned \texttt{pdflatex} compiler. 

An alternative to create documents conform with the above standards also mentioned in~\cite{XMPlatex} is package \pkg{hyperxmp} 
described in~\cite{HyperXmpP}. 
Also, this manual is worth reading, because it gives some overview over XMP metadata 
Although, its focus is on metadata, it is not difficult to set up a \LaTeX{} document to make it PDF/A conform 
setting the appropriate metadata, ensuring that the fonts are embedded and restricting the color scheme. 
In fact, \tool{pdflatex} embeds \pkg{lmodern} fonts loaded by the according package 
whereas the more modern compiler use embedded fonts without further action. 

Nevertheless, the modern technique introduced in~\cite{XMPlatex} to reach these standards 
is by the command \cmd{DocumentMetadata}, which must precede the command \cmd{documentclass}, 
and it provides a list of key, value pairs, described in~\cite{DocMetaDataSuppCode25}, Section~3. 
As noted in~\cite{L3PdfMeta}, Section~2, packages like \pkg{pdfx} and \pkg{hyperxmp} are incompatible. 
At time of this writing, this \LaTeX{} builder imposes the additional restriction, that \cmd{DocumentMetadata} with argument 
must be on a single line. %TBD: eliminate this. 

The most crucial key for \cmd{DocumentMetadata} is \texttt{xmp} 
with boolean value which specifies whether the target PDF file shall contain XMP metadata. 
Since the default is \texttt{true}, the mere existence of the command \cmd{DocumentMetadata} writes XMP metadata, 
which is a prerequisite for all PDF standards. 
To create for example a document claiming PDF/A-4f, 
just write 
%
\begin{Verbatim}[fontsize=\small]
\DocumentMetadata{lang=en-US,pdfversion=2.0,pdfstandard=A-4f}
\end{Verbatim}
%
Here, \texttt{lang=en-US} is just to avoid a warning, and \texttt{pdfversion=2.0} is a recommendation of the author, 
which makes the PDF version explicit and leads attention to fit between PDF version and the PDF/A standard specified. 
As such, \texttt{pdfstandard} just writes the appropriate XMP metadata. 
Besides the claimed conformance \texttt{A-4f}, a lot of material, the \LaTeX{} compiler can provide itself, 
but it is also the title which must be given. 
It is not sufficient, to give the title with \cmd{title}, but it must be available as metadata. 
So use package \pkg{hyperref} and specify 
%
\begin{Verbatim}[fontsize=\small]
\hypersetup{
  pdfinfo={
...
\end{Verbatim}
%
It hurts the single source principle to give author and title twice 
so we recommend using \pkg{hyperref} with option \texttt{pdfusetitle} 
which transfers not only the title but also the author to metadata 
and with \cmd{DocumentMetadata} it occurs in the XMP metadata. 
The above setup can then be reduced: 
%
\begin{Verbatim}[fontsize=\small]
\hypersetup{
  pdfinfo={
...
\end{Verbatim}
%
Strictly speaking these pieces of information are not strictly required by PDF/A, 
but sign of quality. 

% TBD: tell about which of the PDF/A standards are supported. 
% Maybe for all standards together in a table. 

To reach conformance with PDF/A-4f, \tool{pdflatex} must be supported in embedding fonts 
by including package \pkg{lmodern}, whereas more modern compilers do not need any help here. 
\medskip

To reach PDF/UA conformance, besides metadata, tagging is required. 
Essentially, there is a traditional declaration 
%
\begin{Verbatim}[fontsize=\small]
\DocumentMetadata{lang=en-US,pdfversion=1.7,tagging=on,pdfstandard=UA-1}
\end{Verbatim}
%
and a more progressive one, 
%
\begin{Verbatim}[fontsize=\small]
\DocumentMetadata{lang=en-US,pdfversion=2.0,tagging=on,pdfstandard=UA-2}
\end{Verbatim}

Note that without further means, the latter introduces a problem with reproducibility, 
because by default a namespace is added with random name. 
In this manual, we avoided this specifying 
%
\begin{Verbatim}[fontsize=\small]
\DocumentMetadata{lang=en-US,pdfversion=2.0,tagging=on,tagging-setup={role/user-NS=latex},pdfstandard=UA-2}
\end{Verbatim}
%
For a more comprehensive discussion, see Section~\ref{sec:chkReprod}. 


TBD\@: \cmd[colorprofile]{DocumentMetadata}


As already pointed out in Section~\ref{subsec:stdPdf}, 
a PDF document may conform with different standards 
as long as they do not belong to the same family. 
For example to combine PDF/A-4 with PDF/UA-2 one may write 
%
\begin{Verbatim}[fontsize=\small]
\DocumentMetadata{lang=en-US,pdfversion=2.0,tagging=on,pdfstandard={UA-2,A-4}}
\end{Verbatim}
%
or one may repeat the key \texttt{pdfstandard} like so: 
%
\begin{Verbatim}[fontsize=\small]
\DocumentMetadata{lang=en-US,pdfversion=2.0,tagging=on,pdfstandard=UA-2,pdfstandard=A-4}
\end{Verbatim}

Specifying two standards of the same family 
%
\begin{Verbatim}[fontsize=\small]
\DocumentMetadata{lang=en-US,pdfstandard=A-2b,pdfstandard=A-4}
\end{Verbatim}
%
the latter overwrites the former. 
Note that the PDF version is detected from the part of the standard; 
here it would be PDF 2.0 being aligned with PDF/A-4. 

% TBD: here, PDF/X shall be described. 
% First what~\cite{DocMetaDataSuppCode25}, Section~3 says. 
% Then that this is not true 
%\DocumentMetadata{pdfversion=2.0,_pdfstandard/UA-2,_pdfstandard/A-4,_pdfstandard/X-6n,tagging=on,lang=en-US}%xmp=true,



The \LaTeX{} compiler do not know all these specifications in detail, 
sometimes the conformance level cannot be specified, sometimes it must be defined. 
Also, there are case where the compiler cannot guess an appropriate 

This software uses \tool{verapdf} for verification 
and verification is thus restricted to PDF standards PDF/A and PDF/UA at time of this writing. 
For a list of supported parts and levels invoke \texttt{verapdf -l}. 



As described in %Section~\ref{****}, 
\cmd{DocumentMetadata} among other things, allows specifying the PDF version and the PDF standard, 
as e.g. \texttt{a-2b}. 
This is the so called \emph{claimed standard}, 
which is stored in the XMP metadata standardized in~\cite{ISO16684-2} of the PDF artifact. 
An example for such metadata which can be retrieved via \texttt{pdfinfo -meta xxx.pdf} follows. 

Note that \LaTeX{} does not support PDF/A-1a. 

% \begin{lstlisting}
%    <pdf:PDFVersion>1.7</pdf:PDFVersion>
%    <pdfaid:part>2</pdfaid:part>
%    <pdfaid:conformance>B</pdfaid:conformance>
%    <pdfaid:year>2011</pdfaid:year>
% \end{lstlisting}

Because of this metadata \tool{verapdf} applied to the PDF file, 
knows the precise claimed standard(s) with part and level to be verified 
and if verification succeeds, returns whether the document is conforming. 
Verification is needed to show that the document conforms with the standard claimed. 

This software can apply the validator itself, or it can create a \texttt{.latexmkrc} file 
which makes \tool{latexmk} invoke the validator. % TBD: more precise, add text on injections. 
Note that the validator is invoked only, if \cmd{DocumentMetadata} in the \LaTeX{} main file specifies 
one or more standards. 
So in the course of document development, \tool{latexmk} performs verification and displays whether the document passed. 

Note also, that by default, a successful reproducibility check as described in Section~\ref{sec:chkReprod} 
is seen as an indirect validation because the original PDF file is assumed to be conforming. 
Note that this is valid only if reproducibility checks follow the hard strategy of bytewise equality. 
Section~\ref{sec:verapdf} allows configuring the validator to some extent 
and in particular the parameter \texttt{verifyByCmp} described there 
controls whether a reproduced PDF artifact substitutes the invocation of a validator tool. 


\subsection{Verification of PDF standards}\label{subsec:stdVeri}

Independent of the technique to create PDF output, 
verification of the reached standards is essential: 
PDF generators normally ensure, that a requested standard is claimed, 
but hardly do any verification by themselves and \LaTeX{} engines are no exception to that rule. 

As indicated in Section~\ref{subsec:stdDecl}, 
\LaTeX{} engines allow to declare the version of PDF output and more than that, 
conformance with standards PDF/A, PDF/UA and PDF/X by means of \cmd{Documentmetadata}. 
Whereas at time of this writing, no free validator for PDF/X seems available, 
\tool{verapdf} is a renowned open source and free tool to verify PDF/A and PDF/UA\@. 

Although Since PDF files claim performance, including part and conformance level, 
it is not sensible to 
It uses check on the standard~\cite{WtPDF1} on well-tagged PDF to verify PDF/UA-2 
if conformance to the latter is declared. 
To be more precise, it uses both conformance level, WtPDF1r and WtPDF1a. 
If both, some PDF/A and PDF/UA-2 are claimed, then four standards are verified. 

What is strange, if no conformance is claimed, neither PDF/A nor PDF/UA, 
\tool{verapdf} nevertheless verifies PDF/A-1b which fails because conformance is not even claimed. 
This can be explained only if we assume that \tool{verapdf} is used to check PDF/A only 
and if we assume PDF/A-1b the lowest conformance, which is strictly speaking not the truth. 
So, this is just a bug. 
I



% TBD: not only checking, also reaching 

\section{Checking reproducibility}\label{sec:chkReprod}

TBD: rework: also reaching reproducibility. 
Role of timestamp and time zone. 
Trailer ID and what it may depend on. 
In particular, isolating the environment. 
Special care for tagging with PDF 2.0. 

Another source of random is tagging in PDF 2.0 specified for example via 
%
\begin{Verbatim}[fontsize=\small]
\DocumentMetadata{pdfversion=2.0,tagging=on}
\end{Verbatim}
%
This adds a namespace with random name. 
Two techniques avoid this. 
The first is to set the random seed: 
%
\begin{Verbatim}[fontsize=\small]
\UseName{sys_gset_rand_seed:n}{42}
\DocumentMetadata{pdfversion=2.0,tagging=on}
\end{Verbatim}
%
This keeps the namespace random in a sense but makes it reproducible. 
The more brutal way is 
%
\begin{Verbatim}[fontsize=\small]
\DocumentMetadata{pdfversion=2.0,tagging=on,tagging-setup={role/user-NS=latex}}
\end{Verbatim}
%
setting the namespace hardcoded and no longer random. 
As described in~\cite{DocMetaDataSuppCode25}, Section~3, 
\texttt{tagging-setup}'s value is itself a map accepting keys comprising those valid for \cmd{tagpdfsetup} 
defined in package \pkg{tagpdf}. 
The according manual~\cite{tagpdfP}, Table~1, is also the key \texttt{role/user-NS} 
so this can be used also as key as of \texttt{tagging-setup} as in the above \cmd{DocumentMetadata}. 
...
More general, 
if you use this builder to create the manual for a software product and in particular, 
if parts of the text is created by the software for illustration 
in a way described in Section~\ref{sec:pythontex}, 
then it makes sense to make sure that the manual does not change after internal changes in the software. 
...
\begin{itemize}
  \item
  Make the build process reproducible in a hard sense. 
  The advantage of this approach is that diffing is quite simple, 
  fast and reproducible: 
  it is byte by byte as provided by command \tool{diff}. 
  ...
  One approach is the script \tool{vmdiff} 
  % these scripts must be added to the index 
  % THese are vmdiff and ntlatex
  % note that (de)pythontexW are included already
  % maybe also interesting in a general index: .chktexrc and .latexmkrc 
  % and even 
  described in Section~\ref{subsec:ntlatexVmdiff}
  which combines visibility equivalence 
  with equivalence of part of metadata. 
\end{itemize}%

Since the first technique works very well, it is the one we describe here, 
...
Note that if the PDF file is created from TEX files via DVI/\gls{xdv} files, 
...
We suggest to use \tool{exiftool} to modify the \texttt{CreationDate} 
...
One may check with \tool{pdfinfo}, that really \texttt{CreationDate} is modified. 
...
In fact, \tool{exiftool} uses incremental update specified for the PDF format, 
...
\end{verbatim}
%
modifications done can also be undone unless the PDF file is linearized. 
...
it is shorter. 
So, updating the timestamp of the PDF file in \param{diffDirectory} 
...
copying it the into \param{diffDirectory} solves the problem: 
Next compilation yields a PDF file with the correct timestamp and timezone, 
and it coincides with the PDF file in \param{diffDirectory}. 
...
One could place any PDF file in \param{diffDirectory}, 
...
\section{Alternative build process with \protect\toolg{latexmk}}\label{sec:latexmk}%\protect\tool{latexmk}
\sindex[tool]{latexmk}
...
% TBD: at least DVI/\gls{xdv} is possible as well 
...
Another advantage being able to run \tool{latexmk} from within this \LaTeX{} builder: 
It is conceivable, that the artifacts created in the course of document development 
using \tool{latexmk} cannot be reproduced by this builder in the final build. 
The problem is not the difference in the build algorithm, 
because they shall both lead to the same result, 
but because the \LaTeX{} compiler and the other tools may be invoked differently. 
Most likely because \texttt{.latexmkrc} 
does not reimplement the internal functionality of this builder properly. 
Note that \texttt{.latexmkrc} is essentially a reimplementation of neuralgic elements 
of the internal build process in Perl. 
Invoking \tool{latexmk} in a final build for critical documents reduces this risk to a minimum. 
...
should be a picture 
  ...
  \UseTaggingSocket{extFigAlt}%
{Conversion of a TEX file into an XML file with tex4ht.}%
...
should be a picture 
  ...
  \UseTaggingSocket{extFigAlt}%
{Conversion of a TEX file into a DOCX file using odt2doc.}%
...
should be a picture 
  ...
  \UseTaggingSocket{extFigAlt}%
{Conversion of a TEX file into a TXT file using pdftotext.}%
