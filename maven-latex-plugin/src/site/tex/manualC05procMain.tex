...
It goes beyond these standards supporting PDF/E, which has just one part, PDF/E-1, applies from PDF 1.6 on 
and is superseded by PDF/A-4e, but which requires PDF 2.0. 
Another standard not mentioned above is PDF/VT the various parts of which 
are superseded by according PDF/X standards. 



An alternative to create documents conform with the above standards is package \pkg{hyperxmp} 
described in~\cite{HyperXmpP} although, its focus is on metadata. 

Nevertheless, the modern technique to reach these standards 
is by the command \cmd{DocumentMetadata}, which must be the first command in a \LaTeX{} main file 
even in front of \cmd{documentclass}, 
and it provides a list of key, value pairs, described in~\cite{DocMetaDataSuppCode}, Section~3. 
As noted in~\cite{L3PdfMeta}, Section~2, packages like \pkg{pdfx} and \pkg{hyperxmp} are incompatible 
but still their manuals prove helpful. 
