...
% note that running with latexmk neglects choice of latex compiler in magic comment. 
% this requires a warning. 
% In contrast, magic comments referring to the target and related are respected 
...
% special for latexmk: may not touch pdf file although all ok: 
% return code is stil 0 

% pdf files created with latexmk are not suitable for strict diffing: 
% they contain the date. 
% If this functionality is added, then ntlatex is no longer mandatory. 
% This affects also the documentation. 

% latexmk is never used for cleanup with latexmk -c or latexmk -C 

% running latexmk does not respect magic comments: neither target nor compiler. 
% well, maybe target yes. have a look and document. 
% seemingly also restrictions due to the class are respected. 

% eliminate: 
% [INFO] Converting into pdf:  LaTeX file '/home/ernst/OpenSource/LatexMavenPluginGit/latex-maven-plugin/maven-latex-plugin/src/site/tex/./testMagicComments/testProgram.tex'. 
% [INFO] Running latexmk bypassing direct compilation. 
% [ERROR] EEX01: Running latexmk failed with return code 12. 
