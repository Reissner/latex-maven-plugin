...
For each occurrence of a command \cmd{cite\{\textlangle key\textrangle\}} in the TEX file, 
referring to a document with given key,
\texttt{latex2pdf} writes an according entry \cmd{citation\{\textlangle key\textrangle\}} 
into an AUX file. 
Note that, 
...
We run \tool{bibtex} if either 
the \cmd{bibliography}-command, or \cmd{bibliographystyle}-command 
...
For example \cmd{index\{ant-task\}} occurring on page 3 creates an entry 
%
