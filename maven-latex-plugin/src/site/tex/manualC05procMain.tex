...
Modern \LaTeX{} compiler support creating PDF artifacts conform with these standards 
...
conform with more special standards like PDF/A and to verify conformance. 
...
The main fields of industrial applications are those with legally restriction admission processes 
...
the PDF association published a standard on well tagged PDF,~\cite{WtPDF1} 
...
which suggest a reasonable way to satisfy a considerable set of requirements of the PDF/UA standard. 
...
Note that the PDF association published a series of introductions to ISO standards 
...
In reality, all specializations of the PDF standard mentioned above, 
are not single standards but families of standards consisting of several \index{part}\emph{part}s 
...
With Part 4 given by~\cite{PdfA4}, which requires PDF 2.9, the system changes: PDF/A-4 makes sense in itself, 
i.e.\@ the absence of a conformance level is in a sense a kind of implicit conformance level 
and there are extensions PDF/A-4e (flat) and PDF/A-4e (engineering) 
...
and a for accessible, whereas either part of PDF/UA defines a conformance level. 

Table~\ref{tab:versPdfStd} shows the possible combinations. 
...
that so called \emph{claimed standard}, 
...
Declared conformance with a part is a prerequisite for conformance, 
but it is not sufficient. 
Nevertheless, this excludes conformances with two different parts. 
