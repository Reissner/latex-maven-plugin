...
Section~\ref{sec:verify} is special in that it is not related with conversion, 
...
Section~\ref{sec:chkReprod} is also on checking but 
...
\section{Verifying}\label{sec:verify}

In particular in safety and security sensitive fields of application, 
frequently the artifacts are not only required to be in a particular format, 
but shall satisfy particular standards. 
Also, the print industry and organizations which want to provide accessible documents 
impose particular standards. 

For the PDF format, which is a standard in itself, 
as e.g.\@ PDF 1.7 (see~\cite{Pdf17}) or PDF 2.0 described in~\cite{Pdf20}, 
the newest standard at time of this writing, 
the sub-standards families PDF/A, for archiving, 
PDF/X (from eXchange) for print and PDF/UA for unlimited accessibility are defined. 

Note that these families consist of individual parts, 
partially tied to an according PDF version. 
For example, the PDF/A-2 is tied to PDF 1.7. 
Parts 1,2 and 3 have conformance levels b for basic and a for accessible. 
In addition, Parts 2 and 3 have the level u for Unicode. 
With Part 4 the system changes: PDF/A-4 makes sense in itself and there are extensions 
...
%          <pdfd:claimDate>2025-11-28</pdfd:claimDate>
%         </rdf:li>
%        </rdf:Bag>
%       </pdfd:claimData>
%      </rdf:li>
...
that certain pieces of metadata are available in the XMP (Extensible metadata platform) format. 
This is described in a series of norms, 
but for an overview, 
