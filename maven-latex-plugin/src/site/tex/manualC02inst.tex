...
The ant-task requires \tool{ant} and is tested with version 
%
\begin{verbatim}
...
displayed by that release) and the maven-plugin with \tool{mvn}\sindex[tool]{maven|see{mvn}} 
%
\begin{verbatim}
Apache Maven 3.9.2 
\end{verbatim}
...
the interpreter \tool{mpost} or equivalent is required. 
This comes with a standard \TeX{} installation. 
With the standard configuration, 
the resulting EPS-file can be viewed with \tool{ghostscript} 
and for developing it is recommended to have \tool{ghostscript} installed. 
\item
To include \gls{svg}-files into \LaTeX\index{svg}, 
\tool{inkscape} must be installed. 
...
Note that if no pictures created by \tool{fig2dev}, \tool{gnuplot}, 
\texttt{mpost} or by \tool{inkscape} are used, of course, 
neither \tool{fig2dev} nor \tool{gnuplot},
\tool{mpost}, \tool{inkscape} 
nor \tool{dvipdfmx} is needed. 
To include graphics, the graphics bundle described in~\cite{GraX} is required, 
except for SVG-files which requires the package \pkg{svg} 
...
\lualatex, \xelatex{} or \pdflatex. 

\LaTeX{} uses several auxiliary programs. 
Above all \tool{bibtex}, to create the bibliography 
and \tool{makeindex} 
and \tool{splitindex} for the index 
and \tool{makeglossaries} for the glossary. 
The latter two 
...
Note that \texttt{makeglossaries} either invokes \tool{makeindex} 
or \tool{xindy}, depending on the parametrization of the package \pkg{glossaries}. 
Both, \tool{makeglossaries} and \tool{xindy} are written in Perl, 
...
\tool{pythontex} is needed; 
to eliminate that code creating an equivalent TEX file, 
one has to combine it with \tool{depythontex}. 
...
from \LaTeX-files, \tool{htlatex} or alternatively \tool{htxelatex} is used. 
...
the second one is done by \tool{htlatex} or that like. 
% TBD: unification in the long run neither htlatex nor htxelatex but makeht \index{htxelatex} 
\item
To create RTF-files, currently \tool{latex2rtf} is used. 
...
via the command \tool{odt2doc} and thus require three steps 
...
So, for that task, besides \tool{pdftotext} the whole tool chain to create
PDF-files is required. 
\item
An application which does not create a target, 
i.e.~a file in the target directory is \tool{chktex} 
...
\item[\pkg{showlabels}] described in~\cite{ShowLabelsP} 
which displays the defined labels in the text. 
This is used for drafts only. 
...
\begin{itemize}
  ...
  from HTML, ODT, DOC and others, based on DVI/\gls{xdv}\@. 
  \item 
...
which allows configuring checks and the output formats in setting \param{targets},  
...
The goal \texttt{inj} injects files into the working directory \param{texSrcDirectory} 
...
which allows a single parameter, \param{injections}. 
...
cleanup may be deactivated by setting \param{cleanUp} 
...
the parameter \param{cleanUp} is given in the general settings. 
