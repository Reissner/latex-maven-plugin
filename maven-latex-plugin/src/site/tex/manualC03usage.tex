...
% Although the results shall be the same, 
% there is a difference in the build process concerning mostly graphic files: 
...
% On the other hand, if a lot of graphic files are present, 
% \texttt{latexmk} is forced to run the latex generator frequently, 
% interrupting the generator run for each graphic files to be generated, 
% generating the graphic file and rerunning. 
% In contrast, this plugin first creates all graphic files and then invokes the latex generator. 
% Encapsulating graphic files into \cmd{IfFileExists} is not really elegant but prevents \texttt{latexmk} from frequent reruns. 
% The \LaTeX{} sources of this manual use \cmd{IfFileExists} for a listing of a generated graphic file. 
% Unlike for inclusion of graphic files for being interpreted, in case of a listing \cmd{IfFileExists} is mandatory 
% and not only a means to speed up build with  \texttt{latexmk}. 
