...
or executed via the package \pkg{pythontex}. % chktex 24
\end{itemize}


\subsection{\texorpdfstring{\LaTeX{} main files}{LaTeX main files} and other latex files}%\texorpdfstring{\glspl{gls:lmf}}{LaTeX main files}
...
\begin{itemize}
  ...
  \item 
  the parameter \param{patternLatexMainFile} is changed 
\end{itemize}
%
If you distrust the recognition mechanism via pattern matching altogether, 
you can explicitly specify each \LaTeX{} main file 
in the parameter \param{mainFilesIncluded} 
described in Table~\ref{tab:paramGen} on page~\pageref{tab:paramGen}. 
This is safe because if a file specified in \param{mainFilesIncluded} 
that like is not a \LaTeX{} main file according to the pattern \param{patternLatexMainFile}, 
...
but magic comments are deferred to Section~\ref{subsubsec:openingMagComm}. 
...
Nevertheless, the pattern \param{patternLatexMainFile} 
...
For these, we suggest something like\sindex[cmd]{documentclass} 
%
\begin{Verbatim}
\documentclass[a4paper,12pt,USenglish]{article}
\end{Verbatim}
%
...
like so \cmd[{[english]}\{babel\}]{usepackage}, 
...
Note that there are languages with variants like English, 
but then it makes more sense to specify which kind of English, 
so maybe \texttt{english-us} or \texttt{english-uk} 
although unspecific \texttt{english} is possible also. 
Then problem is, that it may depend on the configuration what kind of English this means. 
Note that \texttt{english-US} corresponds with language code \texttt{en-US}, 
\texttt{english-UK} with \texttt{en-UK} and \texttt{english} just with \texttt{en}. 
% TBD: mention \DocumentMetadata lang=en
Unfortunately, at time of this writing, %TBD check, bugreport filed already 
%TBDL abbreviation ldf and ini
the package \pkg{csquotes} does not support this kind of language specifications 
based on ini\footnote{maybe short for initialization} files (see~\cite{BabelP24} Sections~1.8) 
but it supports the more traditional ones based on ldf\footnote{language definition file} files 
described in~\cite{BabelP24} Sections~1.1, 
like \texttt{USenglish} and \texttt{UKenglish} 
together with the less systematic \texttt{british} and \texttt{american}. 

For German, language code \texttt{de} it is similar, as there are three locales, 
\texttt{de-DE}, \texttt{de-AT}, \texttt{de-CH}, for German in the Federal Republic of Germany, 
Austria and Switzerland (Confoederatio Helvetica), respectively. 
By the way, Liechtenstein's official language is German as well with language code \texttt{de-LI}, 
but from \pkg{babel} point of view it is the Swiss variant. 
Likewise, German is one of the official languages in Luxembourg with code \texttt{de-LU} 
but \pkg{babel} treats it as \texttt{de-DE}. 
But compared to English, for the German language, 
things are even more complicated: 
There is a traditional spelling and hyphenation pre-1996 and a ``new'' one since 1996 with some later modifications. 
Then there are two ways to specify ldf and ini (maybe for initializing?). 
In ini style specifications, all goes smooth: \texttt{german} is the modern language, \texttt{german-traditional}, 
like \texttt{chinese} and \texttt{chinese-traditional}. 
As \pkg{csquotes} does not support these names currently, 
we refrain from further discussion. 
% Consequently, 
% for the other traditional variants use \texttt{german-austria-traditional} and \texttt{german-switzerland-traditional}, 
% respectively and for the contemporary ones use \texttt{german-austria} and \texttt{german-Switzerland}. 
% The only bad thing is, that 


In ldf specifications supported by \pkg{csquotes} in contrast, 
new German is called \texttt{ngerman} whereas \texttt{german} is the traditional one. 
It is absurd, that the current language is called new. 
But even worse: if you use \texttt{german} then the ldf is used even if the ini is present on the system, 
but at least ldf takes precedence. 
% To be sure to get the correct variant, the author recommends using \texttt{ngerman} (definitively ldf) 
% and \texttt{german-traditional} (definitively ini). 
The new writing for Austria and Switzerland are \texttt{naustrian} and \texttt{nswissgerman}, 
the traditional for Austria is \texttt{austrian} which fits \texttt{german}, 
but although \texttt{swissgerman} exists, this is a dialect not official German language, 
and the traditional Swiss variant is called  \texttt{swisshighgerman}. 
This is all very confusing. 

% % We hope that once \texttt{german-de} is defined and means contemporary German 
% % in analogy to \texttt{german-at} and \texttt{german-ch} for contemporary Austrian and Swiss German. 
% % This would unify naming of contemporary German a lot. 


% % Although we do not recommend \texttt{swissgerman} be aware that this is in fact not traditional German 
% % in the variant of Switzerland, it is a particular German dialect spoken in Switzerland. 
% % In contrast, \texttt{swisshighgerman} is just \texttt{nswissgerman}. 
% % Of course in Switzerland also \texttt{swissfrench} which is \texttt{french-ch} is spoken. 
% % Also, \texttt{italian} is used but \pkg{babel} treats it the same as the language spoken in Italy, although 
% % language codes \texttt{it} and \texttt{it-CH} exist. 
...
\end{Verbatim}
%
Thus the pattern \param{patternLatexMainFile} 
...
except special options like \texttt{notumble}\sindex[cmd]{documentclass}. 
...
except for the special specification of font size and versioning of the class\sindex[cmd]{documentclass}: 
...
the proper presentation, e.g.\@\sindex[cmd]{documentclass} 
...
What follows is a full range of magic comments\sindex[cmd]{documentclass}: 
%
\begin{Verbatim}
...
in the course of explaining the pattern \param{patternLatexMainFile}. 
...
only the targets \texttt{pdf} and \texttt{dvi} including also \gls{xdv} files. 
...
through settings \param{targets} and \param{docClassesToTargets} 
both in Table~\ref{tab:paramGen} on page~\pageref{tab:paramGen}. 
Also, if a document class occurs, 
which is not registered in \param{docClassesToTargets}, 
...
but they all must match those in pattern \param{patternCreatedFromLatexMain} 
described in Table~\ref{tab:paramGen} on page~\pageref{tab:paramGen}. 
...
injecting files in the \TeX{} source directory and 
...
whereas \xelatex{} creates the \gls{xdv} (extended DVI) format for target \texttt{dvi}. 
...
is given in setting \param{docClassesToTargets} given in 
...
if a document class cannot be identified by the command \cmd{documentclass} 
or the outdated \cmd{documentstyle}, 
...
\param{targets} and in \param{docClassesToTargets}. 
...
overwriting the general setting \param{latex2pdfCommand} in the pom 
...
does not allow specifying a target on the command line. 
...
according to parameter \param{convertersExcluded}, 
which are not installed, i.e.\@ not found, 
and for each of the rest, the actual version, the allowed range 
and a warning if the actual version is out of range. 
Note that the listing is not the copy of a real output for goal \texttt{vrs}: 
\tool{latexmk} is listed as not found but at the same time gives its version. 
This is just illustrative. 

The parameter \param{convertersExcluded} 
...
indicating the converter \tool{pythontexW} with category \texttt{pythontex}. 
...
according to the configuration \param{convertersExcluded}.
...
% According to \cite{UpMendex109}, \cite{MkIdxMoe}
...
[INFO] --- latex:2.1-SNAPSHOT:vrs (default-cli) @ latex-maven-plugin ---
[INFO] Manifest properties: 
[INFO] MANIFEST: (1.0)
[INFO]        Implementation-Version: '2.1-SNAPSHOT'
[INFO] PackageImplementation-Version: '2.1-SNAPSHOT'
[INFO] pom properties: coordinates
[INFO] groupId:    'eu.simuline.m2latex'
[INFO] artifactId: 'latex-maven-plugin'
[INFO] version:    '2.1-SNAPSHOT'
[INFO] git properties: 
[INFO] build version:  '2.1-SNAPSHOT'
[INFO] commit id desc: 'latex-maven-plugin-2.0-488-g3bc2b64-dirty'
[INFO] buildTime:      '2025-11-22T23:05:09+0100'
[INFO] tool versions: 
[INFO] ?warning? command:            'actual version'(not)in[expected version interval]
[INFO]           mvn:                '3.9.11'in[3.9.1;3.9.11]
[INFO]           ant:                '1.10.15'in[1.10.12;1.10.15]
[INFO]           java:               '17.0.17'in[17.0.9;17.0.17]
[INFO]           python:             '3.13.9'in[3.10.12;3.13.9]
[INFO]           perl:               '5.42.0'in[5.34.0;5.42.0]
[INFO]           lua:                '5.4.8'in[5.4.7;5.4.8]
[INFO]           pdflatex:           '1.40.28'in[1.40.21;1.40.28]
[INFO]           lualatex:           '1.22.0'in[1.12.0;1.22.0]
[INFO]           xelatex:            '0.999997'in[0.999992;0.999997]
[INFO]           latex2rtf:          '2.3.18 r1267'in[2.3.16 r1254;2.3.18 r1267]
[INFO]           odt2doc:            '0.9.0'in[0.9.0]
[INFO]           dvips:              '2025.1'in[2020.1;2025.1]
[INFO]           dvipdfm:            '20250410'in[20210318;20250410]
[INFO]           dvipdfmx:           '20250410'in[20200315;20250410]
[INFO]           xdvipdfmx:          '20250410'in[20200315;20250410]
[INFO]           dvipdft:            '20090604.0046'in[20090604.0046]
[INFO]           gs:                 '10.06.0'in[9.52.0;10.06.0]
[INFO]           chktex:             '1.7.9'in[1.7.8;1.7.9]
[INFO]           verapdf:            '1.28.2'in[1.28.2;1.28.2]
[INFO]           diff-pdf-visually:  '1.8.1'in[1.6.4;1.8.1]
[INFO]           diff-pdf:           '300'in[300]
[INFO]           diff:               '3.12'in[3.8;3.12]
[INFO]           pdftotext:          '25.09.1'in[21.04.0;25.09.1]
[INFO]           pdfinfo:            '25.09.1'in[22.01.0;25.09.1]
[INFO]           exiftool:           '13.33'in[12.39;13.33]
[INFO]           bibtex:             '0.99d'in[0.99d]
[INFO]           bibtexu:            '4.02'in[4.00;4.02]
[INFO]           bibtex8:            '4.02'in[4.00;4.02]
[INFO]           makeindex:          '2.17'in[2.15;2.17]
[INFO]           upmendex:           '1.11'in[0.54;1.11]
[INFO]           xindex:             '0.64'in[0.55;0.64]
[INFO]           splitindex:         '0.1'in[0.1]
[INFO]           makeglossaries:     '4.7'in[4.45;4.7]
[INFO]           bib2gls:            '4.7'in[4.4;4.7]
[INFO]           pythontex:          '0.18'in[0.17;0.18]
[INFO]           depythontex:        '0.18'in[0.17;0.18]
[INFO]           latexmk:            '4.87'in[4.70b;4.87]
[INFO]           mpost:              '2.11'in[2.00;2.11]
[INFO]           extractbb:          '1.1.0'in[1.1.0]
[INFO]           gnuplot:            '6.0 patchlevel 3'in[5.4 patchlevel 0;6.0 patchlevel 3]
[INFO]           inkscape:           '1.4.2'in[1.0.2;1.4.2]
[INFO]           fig2dev:            '3.2.9a'in[3.2.7b;3.2.9a]
[INFO] tools excluded: 
[INFO] xindy
...
[INFO] ------------------------------------------------------------------------
[INFO] Finished at: 2025-11-22T23:10:27+01:00
\end{lstlisting}
%[INFO]           upmendex:           '1.00'in[0.54;1.00]
%[INFO]           xindy:              '2.5.1'in[2.5.1]
% TBD: clarify whether xindy is really to be excluded. 
% It is still used indirectly invoked by makeglossaries. 
...
Of particular interest is Table~\ref{tab:WarnALP} on failures running a tool 
...
\begin{itemize}
  ...
  \item
  errors and warnings in log files (provided by many but not all tools). 
...
into the working directory \param{texSrcDirectory}, 
during a maven lifecycle, by default in phase \texttt{valiate}, 
or from command line invoked from the root directory and injecting into that root. 
The files injected into \param{texSrcDirectory} 
are in general adapted to the current configuration of the plugin; 
the others are auxiliary files valid in any project using this \LaTeX{} plugin. 
...
Accordingly, as described in Section~\ref{subsec:develClear}, 
the files not overwritten by goal \texttt{ini} 
are precisely the files not erased by goal \texttt{clr}. 
...
which is necessary to keep them synchronized 
with the configuration of this plugin, 
...
\begin{itemize}
  ...
  reflecting the settings of this plugin 
  and form the default for \tool{latexmk} and for \texttt{chktex}, 
  i.e.\@ behavior without command line options. 
  ...
  and also of DVI/\gls{xdv} which is an important intermediate format. 
  ...
  At the time of this writing, they are the same for all settings, 
  but adapt to the settings by a system of if-statements. 
  For example the headers are the same, independently of the \LaTeX{} compiler. 
  Instead, the headers detect the compiler by need at runtime. 
  \item
  ...
  invoked from the project root as described below. 
  In contrast to the files in the other categories, these are executable 
  are not injected into \param{texSrcDirectory}. 
  They are not adapted to the configuration but shall be used globally. 
  Thus, it makes sense to make them available for execution 
  by moving them to appropriate places like \texttt{/usr/bin} on Linux systems. 
\end{itemize}
...
\begin{longtable}{l|ll}
 ...
 {\tiny headerSuppressMetaPDF} & \texttt{\tiny headerSuppressMetaPDF.tex} & to control PDF meta-info      \\
  ...
  pythontexW            & \texttt{\footnotesize pythontexW}        & surrogate for \tool{pythontex} \\
  depythontexW          & \texttt{\footnotesize depythontexW}      & surrogate for \tool{depythontex} \\
  \end{longtable}
...
Then the files are injected in the TEX root directory \param{texSrcDirectory}. 
...
The file \href{\urlSite fromTex/.latexmkrc}{.latexmkrc} 
tied to the injection \texttt{latexmkrc} 
is the configuration file for the build tool \tool{latexmk} 
and likewise \href{\urlSite fromTex/.chktexrc}{.chktexrc} 
tied to the injection \texttt{chktexrc} 
...
Thus, \tool{latexmk} is intended to be used with additional options in exceptional cases only. 
Currently, not all possible settings of this plugin 
are taken into account in the \texttt{.latexmkrc}, % TBD: change that 
...
This is true even for creating this manual 
which presents and uses a maximum variety of features of this software. 
...
Still, creation of PDF files is supported in both variants, via DVI/\gls{xdv} or directly. 
...
Among these are also reproducibility checks 
...
it makes sense, to synchronize \texttt{.chktexrc} and \texttt{header.tex}. 

As an example, package \pkg{listings} which provides the environment \texttt{lstlisting} 
and the command \cmd{listinputlisting}. 
The content of both shall not be subject to checks via \texttt{chktex} 
and must thus be excluded in \texttt{.chktexrc}. 
The role of \texttt{header.tex} in this context is, 
to detect whether \pkg{listings} and, if so, to patch it. 
Among other things, this allows to compile the document 
not only with the internal build mechanism of this plugin, 
but also by \tool{latexmk} with good performance. 
...
but they may be invoked by this \LaTeX{} builder 
in the course of a regular build, 
...
Likewise,~\cite{ChkTeX22}, Section 6.1.3, shows that also \texttt{chktex} 
...
\lstinputlisting[linerange={129-166},
...
%
\lstinputlisting[linerange={113-115},
language=tex, basicstyle=\scriptsize, 
...
like in \cmd[\{pdfversion=1.7\}]{DocumentMetadata}, 
but due to a bug, any invocation of \cmd{DocumentMetadata} 
corrupts reproducibility of the created PDFs. 
...
just like \cmd{texttt} but for which checks by \tool{chktex} are suppressed. 
...
%
\begin{itemize}
  \item various \LaTeX{} engines, \lualatex, \xelatex{} and \pdflatex{} 
  ...
  \item for creating PDF files, but further formats created with \pkg{tex4ht} as well
  \item direct creation of PDF and via intermediate DVI/\gls{xdv} 
\end{itemize}
...
which is invoked at \tool{latexmk}'s startup. 
...
Provided the extensions given by \texttt{instVScode4tex.sh} are installed, 
opening \href{\urlSite fromTex/.latexmkrc}{.latexmkrc}, 
which is just Perl code, in VS Code, one can see the highlighting.



\subsection{Scripts in conjunction with reproducibility}%
\label{subsec:ntlatexVmdiff}
...
\end{Verbatim}
%
injects the according files \tool{ntlatex} and \tool{vmdiff} 
in the root directory. 

The injection \tool{ntlatex} 
injects the file \href{\urlSite fromMain/ntlatex}{ntlatex} 
which runs the \LaTeX{} compiler specified in the pom, 
or in the magic comments if present, 
to create a PDF file. 
As usual, magic comments override configuration in the pom. 
Also, it takes into account whether the PDF file is created via intermediate DVI/\gls{xdv} files or not, 
...
It is because \tool{latexmk} won't recompile, 
if the expected PDF file exist already 
and no sources changed. 
So \tool{ntlatex} is needed to force recompilation. 

It is easy to see, that \tool{ntlatex} is helpful also during document development 
for fixing bugs and to detect the cause of compilation errors. 
\medskip

%\index{diff-pdf}
Complementary to this \tool{vmdiff} is a diff tool for PDF files 
combining visual equality checked with \tool{diff-pdf-visually} 
with equality of metadata checked via \tool{pdfinfo} 
...
except \tool{diff-pdf-visually} and \tool{pdfinfo}. 
...
\includegraphics[alt=development process]{F3_01texUsagePlain.mps}
}
...
Among the build ``recipes'' is \texttt{latexmk (latexmkrc)} 
...
As a PDF viewer, we use \tool{okular} 
...
For the viewer \tool{okular}, backward search is configured in tab ``editor'' 
as described above, 
...
\subsection{The build tool \toolg{latexmk}}\label{subsec:develLatexmk}
...
and thus be invoked a priori on the console. 
For FIG files this is \tool{fig2dev}. 
...
and besides the \LaTeX{} engine typically 
...
and whether the \LaTeX{} engine or an auxiliary program 
is to be invoked once more 
...
The PDF file, or whatsoever is created if not present, 
...
Instead, it awaits a change of a source file 
which triggers a new run of some basic tool 
...
\subsubsection{Differences of \toolg{latexmk} with this \LaTeX{} builder}%
...
Another difference is, that by default, 
the plugin cleans up the folder with the TEX sources, 
and only the resulting file, e.g.\@ PDF, 
is copied to the target folder before cleanup. 
To be more precise, 
only the files present before the build are kept, possibly updated, 
all the others are removed. 
This is appropriate for a maven plugin 
but destroys log files containing vital information 
if the build goes wrong. 
Still if a file is interesting, 
it may be created by \tool{touch} or by some basic latex tool 
as \lualatex{} or \tool{makeindex} 
and then a build done by this plugin 
will pertain the file updated by the build process. 
...
This software recognizes whether \texttt{.latexmkrc} is defined by the user 
and, if so, does not overwrite or erase this file. 

Based on injection of \texttt{.latexmkrc}, 
this plugin may even use \tool{latexmk} 
as a means to build bypassing its internal build rules. 
For motivation of this feature 
and for details in implementation see Section~\ref{sec:latexmk}. 

There is a difference in the build processes 
(except if this plugin uses \tool{latexmk}) 
concerning mostly graphic files: 
\tool{latexmk} detects dependencies 
via the \texttt{-recorder} option of the latex generator 
...
\tool{latexmk} requires 10 runs of the \LaTeX{} engine 
if no bibliography or so is presentations. 
In contrast, this plugin requires a single run in this case, 
...
and in some cases is a technique 
to make the build process with \tool{latexmk} work. 
One of these cases, 
...
As this latex builder may invoke \tool{latexmk} 
either in general or for selected files, 
...
It is possible to combine this plugin with \tool{latexmk} 
to speed up \tool{latexmk}: 
%
\begin{Verbatim}
...
Finally, goal \texttt{grp} creates the graphic files 
which speeds up \tool{latexmk} if this is invoked next. 
Of course the above maven invocation is also a good initialization 
for building with the basic tools without \tool{latexmk}. 

Finally, goal \texttt{clr} tied to phase \texttt{clean} 
erases all intermediate files 
...
\subsubsection{How \toolg{latexmk} is integrated}\label{subsubsec:latexmkIntegration}

Finally, we show how this plugin may support \tool{latexmk}. 
...
This software and \tool{latexmk} 
follow a different philosophy in finding dependencies: 
Whereas this software creates image files in advance 
before invoking a \LaTeX{} engine, 
...
and the engine is run again, 
this time passing the inclusion of the first created files 
failing at the next one. 
To find out that another rule is needed, 
\tool{latexmk} parses the LOG file of the latex compiler. 
...
that there is at least one kind of inclusion which does not work that way: 
...
Another point is, that currently for each file 
\tool{latexmk} creates with a separate rule, 
...
% TBD: this mechanism is described more than once. Rework required. 
...
\param{cleanUp} is set to its default \texttt{true}. 
...
Either \param{cleanUp} was set to \texttt{false} 
...
Deletion is driven by a regular expression \param{patternCreatedFromLatexMain} 
described in Table~\ref{tab:paramGen} on page~\pageref{tab:paramGen}. 
...
through \tool{xfig}'s export dialog, 
...
set the parameter \param{cleanUp} in the pom 
...
Besides the basic configuration packaged with \tool{chktex}, 
...
The target formats and also the checks are specified in the parameter \param{targets}. 
