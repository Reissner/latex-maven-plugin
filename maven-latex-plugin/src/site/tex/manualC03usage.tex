...
that like is not a \LaTeX{} main file according to \texttt{patternLatexMainFile}, 
...
This is what we mean saying that \texttt{latexmk} \emph{integrates} the basic tools. 
...
Let us discuss the differences between \texttt{latexmk} and this latex plugin: 
...
Based on injection of \texttt{.latexmkrc}, this plugin may even use \texttt{latexmk} 
...
There is a difference in the build processes (except if this plugin uses \texttt{latexmk}) 
...
\texttt{latexmk} runs the latex converter in \texttt{nonstopmode} mode. 
...
for \texttt{latexmk} only the 10th runs of the converter is completed. 
In contrast, this plugin requires a single run, so performance is significantly better. 
The use of \cmd{IfFileExists} is not really elegant but prevents \texttt{latexmk} from frequent reruns 
and in some cases is a technique to make the build process with \texttt{latexmk} work. 
One of these cases, related with using \pkg{listings} is discussed below in this section. 

More general, there are cases, where this latex builder succeeds but \texttt{latexmk} does not. 
As this latex builder may invoke \texttt{latexmk} either in general or for selected files, 
this latex builder is mightier than \texttt{latexmk}. 
If both approaches succeed, the results shall be the same for this plugin and for \texttt{latexmk}. 
...
Finally, we show how this plugin may support \texttt{latexmk} where. 
...
To find out that another rule is needed, \texttt{latexmk} parses the LOG file of the latex compiler. 
