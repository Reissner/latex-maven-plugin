...
\DocumentMetadata{uncompress,xmp=true,lang=en-US}%
\documentclass[a4paper]{article}%scrartcl [12pt, a4paper]%, english]
...
The names of the underlying engines just drop the inner syllable ``\texttt{la}''. 
For \texttt{pdftex} there is a user manual~\cite{PdfTexUsr24}, 
for \texttt{luatex} there is a reference manual~\cite{LuaTexRef25} and 
for \texttt{xetex} a reference guide~\cite{XeTexRef25}. 
...
The focus is here on PDF files and comprises besides visible data 
also metadata. 
...
but it can also process the options if given with a single dash. 
Conversely, \LaTeX{} engines other than \lualatex{} 
can also deal with options starting with double dash. 
Options unknown to a \LaTeX{} engine never result in an error or even a warning; 
instead just an info message `unrecognized option' is displayed. 
...
Although \xelatex{} always produces a XDV file as an intermediate step, 
...
in Section~\ref{subsubsec:dateTimeZoneTexPdf}, 
Section~\ref{subsubsec:dateTimeZones} provides the general foundation. 


\subsubsection{Date/time formats and time zones}\label{subsubsec:dateTimeZones}

This section introduces time zones, date/time formats and conversions between them. 

We start with \emph{epoch time} 
which is the most important date/time format, although not well human-readable. 
...
\end{verbatim}
%
i.e.\@ the point in time, where in timezone UTC 
(represented by the \texttt Z, zero deviation from UTC, also called Zulu-time) 
...
or some deviation from UTC for example given with two digits for hours and two digits for minutes. 
The same date and time as above in time zone two hours earlier (more east) than UTC writes 
%
\begin{verbatim}
  1970-01-01T02:00:00+0200
\end{verbatim}

In POSIX operating systems, among the standard environment variables is also \texttt{TZ} 
specifying the time zone as described in 
\url{https://www.gnu.org/software/libc/manual/html_node/Standard-Environment.html}. 
...
%
CAUTION\@: 
The number is understood as to be added to given time in the current time zone 
to get UTC, so east of UTC has negative numbers. 
This is the sign opposite to the representation given by ~\cite{ISO8601-1} 
which is the offset to be added to UTC to get the time in the given time zone. 
...
\begin{description}
  ...
  \item[\texttt{date -d 'Apr 29 2024' +\%s}] % chktex 32
...
These are typically displayed by commands like \cmd{date} or \cmd{time}. 
We prefer \cmd{Today} 
provided by package \texttt{datetime2} with option \texttt{style=iso} 
which displays the date in the form \texttt{yyy-mm-dd}. 
This can be used also in the metadata. % TBD: reference missing 
Note that it is not secure to use \cmd{today} because packages overwrite this. 
For experiments, we may use \cmd{printdatetime} defined in Listing~\ref{lst:printdatetime} 
which is based on \cmd{time}. 

\lstinputlisting[linerange={62-73},
language=tex, %basicstyle=\scriptsize,
float, captionpos=b, label={lst:printdatetime}, 
caption={Definition of \cmd{printdatetime} }]{latexEngines.tex}
...
For \pdflatex, the main description is in the introductory text of~\cite{PdfTexUsr25}, Chapter~2.
...
Essentially, \xelatex{} behaves the same as described in\xelatex:~\cite{XeTexRef25}, Sections 8.3 and 8.5. 

In contrast to this, as described in~\cite{LuaTexRef25}, Section 4.4, 
...
The root for metadata in PDF file is the \emph{trailer dictionary}. 
...
To be more precise it is the XMP metadata, which is an XML format, described in~\cite{ISO16684-1:19}. 
The superseded standard~\cite{ISO16684-1:12} is publicly available 
and gives already sufficient insight. 
...
The table does not list all XMP keys specified in~\cite{ISO16684-1:19}, 
but only those which are observed as created by a \LaTeX{} engine. 
%Observe that the XMP keys with namespace \texttt{pdf} are not specified in~\cite{ISO16684-1:19}. % was true for 2012 version
...
\begin{longtable}{lll}
  ...
  ---          & dc:source        & the \LaTeX{} main file \\% TBD: this term shall be in terms.bib. 
...
The package can even not be loaded if \cmd{DocumentMetadata} as shown in \cite{HyperXmpP}, Section~3.1. 
For this task consider post-processing combining \texttt{exiftool} unlinking metainformation and 
\texttt{qpdf} removing by linearizing. 
%https://cyberrunner.medium.com/removing-metadata-from-pdf-files-using-exiftool-and-qpdf-20090b75d7f0
At least, \texttt{pdfinfo} makes meta info visible, questionable whether completely. 
...
For \lualatex{}, 
\cite{LuaTexRef25}, Section 3.2.2 describes the variable \texttt{suppressoptionalinfo} 
...
As described in~\cite{PdfTexUsr25}, Section 4.2.3 and 4.2.4, 
\pdflatex{} can suppress only \texttt{PTEX} data 
...
For \lualatex{}~\cite{LuaTexRef25}, Section 3.2.2 shows how to overwrite the trailer ID\@. 
In contrast, \pdflatex{} has many options to overwrite metadata 
collected in~\cite{PdfTexUsr25}, Section 4.2, among those also the trailer ID\@. 
...
see~\cite{LuaTexRef25}, Section 14.1.8 for \lualatex, 
and~\cite{PdfTexUsr25}, Section 4.2 for \pdflatex. 
...
described in~\cite{ISO16684-1:19} is written. 
...
Readers who refuse using \cmd{DocumentMetadata} and  use \xelatex{} 
may take refuge to switching off compression from within the document via 
%
\begin{verbatim}
  \special{xdvipdfmx:config z 0}
\end{verbatim}

Equivalently one can use the command line 
%
\begin{verbatim}
